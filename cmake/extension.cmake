...
function(add_app_compress target)
    if(NOT EXISTS ${out_inc_dir}/ap_compress_cfg.h)
        file(WRITE ${out_inc_dir}/ap_compress_cfg.h "#define compress_contents")
    endif()
    file(APPEND ${out_inc_dir}/ap_compress_cfg.h " SORT(*lib${target}.a:*.*)(DEF_RO)")
endfunction()
...
macro(pac_init_fdl cmd)
    ...
    if(CONFIG_SOC_8821)
        set(${cmd}
            cfg-init --pname "8821_MODEM" --palias ${BUILD_TARGET}
                --pversion "8821 MODULE" --version "BP_R1.0.0"
                --flashtype 0
            cfg-fdl -a ${CONFIG_NORFDL_IMAGE_START} -s ${CONFIG_NORFDL_IMAGE_SIZE}
                -p ${pac_fdl_files}
        )
...
function(add_appimg target ldscript) # <sources> LINK_LIBRARIES <libs> OUTPUT_DIR <dir>
    ...
    if((CONFIG_SOC_8910) OR (CONFIG_SOC_8811) OR (CONFIG_SOC_8821))
        add_custom_command(TARGET ${target} POST_BUILD
...
function(add_appimg_flash_example target)
    ...
    if((CONFIG_SOC_8910) OR (CONFIG_SOC_8811) OR (CONFIG_SOC_8850) OR (CONFIG_SOC_8821))
        set(pac_file ${out_hex_example_dir}/${target}.pac)
        set(target_img_file ${out_hex_example_dir}/${target}.img)
        set(target_sign_img_file ${out_hex_example_dir}/${target}.sign.img)
...
# quectel start
# Build ql_example appimg2 (linked in flash). Parameters are source files.
function(add_appimg2_flash_ql_example target)
    set(ldscript ${SOURCE_TOP_DIR}/components/apploader/pack/app2_flashimg.ld)
    add_appimg(${target} ${ldscript} EXCLUDE_FROM_ALL ${ARGN}
        OUTPUT_DIR ${out_hex_example_dir})
    add_dependencies(ql_examples ${target})
    set(prepack_cpio ${out_hex_dir}/examples/${target}_prepack.cpio)
    ...
    if((CONFIG_SOC_8910) OR (CONFIG_SOC_8811) OR (CONFIG_SOC_8850))
        ...
        add_custom_command(OUTPUT ${prepack_cpio}
            COMMAND python3 ${modemgen_py} prepackgen
                --source-top ${SOURCE_TOP_DIR}
                --binary-top ${BINARY_TOP_DIR}
                --prepackfile ${CONFIG_PACKAGE_FILE_APPIMG2_JSON_PATH}
                ${prepack_cpio}
            DEPENDS ${modemgen_py}
            WORKING_DIRECTORY ${SOURCE_TOP_DIR}
        )
        if(CONFIG_PACKAGE_FILE_APPIMG2_JSON_PATH)
            set(cfg_pack_gpio cfg-pack-cpio -i PREPACK -p ${prepack_cpio})
            set(prepack_file ${prepack_cpio})
        endif()
...
# Build ql_example appimg2 (linked in file). Parameters are source files.
function(add_appimg2_file_ql_example target)
    set(ldscript ${SOURCE_TOP_DIR}/components/apploader/pack/app2_fileimg.ld)
    add_appimg(${target} ${ldscript} EXCLUDE_FROM_ALL ${ARGN}
        OUTPUT_DIR ${out_hex_example_dir})
    add_dependencies(ql_examples ${target})
    set(prepack_cpio ${out_hex_dir}/examples/${target}_prepack.cpio)
    ...
    if((CONFIG_SOC_8910) OR (CONFIG_SOC_8811) OR (CONFIG_SOC_8850))
        ...
        if((CONFIG_SOC_8910) OR (CONFIG_SOC_8850))
            add_custom_command(OUTPUT ${pac_file}
                COMMAND python3 ${pacgen_py} ${init_fdl}
                    cfg-pack-file -i APPIMG2 -p ${target_sign_img_file}
                    -n ${CONFIG_APPIMG2_LOAD_FILE_NAME}
                    ${cfg_pack_gpio}
                    pac-gen ${pac_file}
                DEPENDS ${pacgen_py} ${pac_fdl_files} ${target_sign_img_file} ${prepack_file}
                WORKING_DIRECTORY ${SOURCE_TOP_DIR}
            ...
            add_custom_command(OUTPUT ${pac_file}
                COMMAND python3 ${pacgen_py} ${init_fdl}
                    cfg-pack-file -i APPIMG2 -p ${target_img_file}
                    -n ${CONFIG_APPIMG2_LOAD_FILE_NAME}
                    ${cfg_pack_gpio}
                    pac-gen ${pac_file}
                DEPENDS ${pacgen_py} ${pac_fdl_files} ${target_img_file} ${prepack_file}
...
function(add_appimg_delete)
    if((CONFIG_SOC_8910) OR (CONFIG_SOC_8811) OR (CONFIG_SOC_8850) OR (CONFIG_SOC_8821))
        if(CONFIG_APPIMG_LOAD_FLASH)
			...
			if(CONFIG_QUEC_PROJECT_FEATURE_APPIMG2) #quectel update
				set(target appimg2_flash_delete)
				set(pac_file ${out_hex_dir}/${target}.pac)
				pac_init_fdl(init_fdl)

				add_custom_command(OUTPUT ${pac_file}
					COMMAND python3 ${pacgen_py} ${init_fdl}
						cfg-erase-flash -i ERASE_APPIMG2
							-a ${CONFIG_APPIMG2_FLASH_ADDRESS}
							-s ${CONFIG_APPIMG2_FLASH_SIZE}
						pac-gen ${pac_file}
					DEPENDS ${pacgen_py} ${pac_fdl_files}
					WORKING_DIRECTORY ${SOURCE_TOP_DIR}
        ...
        if(CONFIG_APPIMG_LOAD_FILE)
			...
			if(CONFIG_QUEC_PROJECT_FEATURE_APPIMG2) #quectel update
				set(target appimg2_file_delete)
				set(pac_file ${out_hex_dir}/${target}.pac)
				pac_init_fdl(init_fdl)

				add_custom_command(OUTPUT ${pac_file}
					COMMAND python3 ${pacgen_py} ${init_fdl}
						cfg-del-appimg -i DEL_APPIMG2
						pac-gen ${pac_file}
					DEPENDS ${pacgen_py} ${pac_fdl_files}
					WORKING_DIRECTORY ${SOURCE_TOP_DIR}
				)
