...
/**
 * \brief write nv item from buffer and update crc on nv tail or head, only considering running data
 *
 * Comparing to \p nvmWriteItem, the difference is this will calculate data crc and write
 * to the nv file, and only write running nv.
 *
 * \param nvid      nv ID
 * \param buf       buffer for nv item write
 * \param size      buffer size
 * \param flag      crc store head/tail flag
 * \return
 *      - nv item size on changed
 *      - -1 on fail
 */
int nvmWriteCrcItem(uint16_t nvid, const void *buf, unsigned size, uint8_t flag);
...
/**
 * \brief read imei.bin to buffer
 *
 * This can be used to read imei\wifi\bt\gps, and imei\wifi\bt\gps is member of
 * imei.bin.
 *
 * When \p buf is NULL or \p size is 0, it will return the size of
 * nv item if the nv item exists.
 ...
 * it will return -1. That is, partial read of nv item is not supported.
 *
 * \param offset     the offset of nv item from imei.bin
 * \param nv_size   nv item size
 * \param buf         buffer for nv item read
 * \param size        buffer size
 * \return
 *      - nv item size on success
 ...
 *         - \p size is less than nv item size
 */
int nvmReadNvDirect(unsigned offset, unsigned nv_size, void *buf, unsigned size);

/**
 * \brief write imei.bin from buffer
 *
 * This can be used to write imei\wifi\bt\gps, and imei\wifi\bt\gps is member of
 * imei.bin.
 *
 * When \p buf is NULL or \p nv_size unequal to size, it will return -1.
 *
 * The content will be checked before write. When the exist contents is
 * the same, no operation will be taken and is regarded as success.
 *
 ...
 *      - -1 on fail
 */
int nvmWriteNvDirect(unsigned offset, unsigned nv_size, const void *buf, unsigned size);
...
/**
 * \brief update nv crc data
 * \param force
 *  -when force is true, the nv item crc will be updated
 *  -when force is false, it will update nv item crc when running nv non-existent
 *
 * \return
 *      0
 */
int nvmUpdateCrcItem(bool force);
...
/**
 * \brief backup nv item
 *
 * \return
 *      0 - sucess;
 *      -1 - fail
 */
int nvmWriteBackupItem(void);
int gSysnvCalibStatus;
#endif

uint16_t calculateCrc16(uint16_t *pBuf, uint16_t len);
