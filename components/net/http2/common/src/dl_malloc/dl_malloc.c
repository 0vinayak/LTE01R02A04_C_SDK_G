...
#else
#include <stdio.h>       /* for printing in malloc_stats */	//bruce move here
...
#endif  /* INSECURE */
#ifndef HAVE_MMAP
//#define HAVE_MMAP 1 //bruce change 1 to 0
#define HAVE_MMAP 0 //bruce
#endif  /* HAVE_MMAP */
#ifndef MMAP_CLEARS
#define MMAP_CLEARS 1
#endif  /* MMAP_CLEARS */
...
#endif  /* HAVE_MREMAP */
#ifndef MALLOC_FAILURE_ACTION
#define MALLOC_FAILURE_ACTION                       //bruce
#endif  /* MALLOC_FAILURE_ACTION */
#ifndef HAVE_MORECORE
    #if ONLY_MSPACES
        #define HAVE_MORECORE 0
    #else   /* ONLY_MSPACES */
        #define HAVE_MORECORE 0 //bruce change from 1 to 0
    #endif  /* ONLY_MSPACES */
#endif  /* HAVE_MORECORE */
    ...
    #if (MORECORE_CONTIGUOUS || defined(DAPS_WIN32))
...
#    else
#      ifdef DAPS_WIN32 /* use supplied emulation of getpagesize */
#        define malloc_getpagesize getpagesize()
#      else
...
static FORCEINLINE void* win32mmap(size_t size) {
  void* ptr = VirtualAlloc(0, size, MEM_RESERVE|MEM_COMMIT, PAGE_READWRITE);  /*lint !e628*/
...
static FORCEINLINE int win32munmap(void* ptr, size_t size) {
  ...
  char* cptr = (char*)ptr;
  while (size) {
    if (VirtualQuery(cptr, &minfo, sizeof(minfo)) == 0)  /*lint !e628*/
      return -1;
    if (minfo.BaseAddress != cptr || minfo.AllocationBase != cptr ||
        minfo.State != MEM_COMMIT || minfo.RegionSize > size)
      return -1;
    if (VirtualFree(cptr, 0, MEM_RELEASE) == 0)   /*lint !e628*/
      return -1;
...
int malloc_corruption_error_count = 0; /*lint -esym(552,malloc_corruption_error_count) */
...
static int init_mparams(void) {
...
#else /* DAPS_WIN32 */
    {
        ...
        GetSystemInfo(&system_info);  /*lint !e628*/
        psize = system_info.dwPageSize;
        gsize = ((DEFAULT_GRANULARITY != 0)?
            DEFAULT_GRANULARITY : system_info.dwAllocationGranularity);
    }
#endif /* DAPS_WIN32 */
    /* Sanity-check configuration:size_t must be unsigned and as wide as pointer type.
       ints must be at least 4 bytes.alignment must be at least 8.
...
#if USE_DEV_RANDOM
        int fd = -1;
        unsigned char buf[sizeof(size_t)] = {0};
...
#endif /* USE_DEV_RANDOM */
#ifdef DAPS_WIN32
            magic = (size_t)(GetTickCount() ^ (size_t)0x55555555U);  /*lint !e628*/
#else
            magic = (size_t)(SCI_GetTickCount() ^ (size_t)0x55555555U);
#endif
...
#if ONLY_MSPACES
#define internal_malloc(m, b) cfl_mspace_malloc(m, b)
#define internal_free(m, mem) cfl_mspace_free(m,mem);
#else /* ONLY_MSPACES */
#if MSPACES
#define internal_malloc(m, b)\
    (m == gm)? dlmalloc(b) : cfl_mspace_malloc(m, b)
#define internal_free(m, mem)\
    if (m == gm) dlfree(mem); else cfl_mspace_free(m,mem);
#else /* MSPACES */
...
static void reset_on_error(mstate m)
{
    ...
    memset( m->smallbins, 0 , sizeof(m->smallbins));
    memset( m->treebins, 0, sizeof(m->treebins));
            ...
            if (base != CMFAIL)
            {
              ...
              {    /* Cope with partial failure */
                  ...
                  {    /* Try to use/extend the space we did get */
                      if (asize < HALF_MAX_SIZE_T && asize < nb + SYS_ALLOC_PADDING)
                      {
    ...
    if (tbase != CMFAIL)
    {
        ...
        { /* first-time initialization */
...
#endif
            {
              ...
              if (sp != 0 && !is_extern_segment(sp) && (sp->sflags & USE_MMAP_BIT) == mmap_flag && segment_holds(sp, m->top)) 
              { /* append */
                  sp->size += tsize;
...
#endif /* !ONLY_MSPACES */
size_t cfl_dlmalloc_usable_size(void* mem) {
    if (mem != 0)
    {
        mchunkptr p = mem2chunk(mem);
        if (is_inuse(p))
...
mspace cfl_create_mspace(size_t capacity, int locked)
{
    mstate m = 0;
    size_t msize;
    ensure_initialization();
...
mspace cfl_create_mspace_with_base(void* base, size_t capacity, int locked)
{
    mstate m = 0;
    size_t msize;
    if(NULL == base || 0 == capacity)
    {
        return NULL;
    }
    ensure_initialization();
...
int cfl_mspace_track_large_chunks(mspace msp, int enable)
{
    int ret = 0;
...
size_t cfl_destroy_mspace(mspace msp)
{
    size_t freed = 0;
    mstate ms = (mstate)msp;
    if (ok_magic(ms))
    {
        ...
        while (sp != 0)
        {
            ...
            sp = sp->next;
            if ((flag & USE_MMAP_BIT) && !(flag & EXTERN_BIT) && CALL_MUNMAP(base, size) == 0)
                freed += size;
        }
    }else{
...
void* cfl_mspace_malloc(mspace msp, size_t bytes)
{
    mstate ms = (mstate)msp;
    if (!ok_magic(ms)) {
        USAGE_ERROR_ACTION(ms,ms);
...
void cfl_mspace_free(mspace msp, void *mem)
{
    if (mem != 0)
    {
        mchunkptr p  = mem2chunk(mem);
...
void* cfl_mspace_calloc(mspace msp, size_t n_elements, size_t elem_size)
{
    void* mem;
    size_t req = 0;
    mstate ms = (mstate)msp;
...
size_t cfl_msapce_is_empty(mspace msp)
{
    mstate m = (mstate)msp;
    size_t used = 0;
    ensure_initialization();
    if (!PREACTION(m))
    {
        ...
        if (s != 0)
        {
            mchunkptr q = align_as_chunk(s->base);
            q = next_chunk(q);
            if (segment_holds(s, q) && q != m->top && q->head != FENCEPOST_HEAD)
            {
                return 0;
            }else{
                return 1;
...
void* cfl_mspace_realloc(mspace msp, void* oldmem, size_t bytes)
{
    if (oldmem == 0)
        return cfl_mspace_malloc(msp, bytes);
#ifdef REALLOC_ZERO_BYTES_FREES
    if (bytes == 0)
    {
        cfl_mspace_free(msp, oldmem);
        return 0;
    }
#endif /* REALLOC_ZERO_BYTES_FREES */
    else {
...
void* cfl_mspace_memalign(mspace msp, size_t alignment, size_t bytes)
{
    mstate ms = (mstate)msp;
    if (!ok_magic(ms))
    {
...
void** cfl_mspace_independent_calloc(mspace msp, size_t n_elements, size_t elem_size, void *chunks[])
{
    size_t sz = elem_size; /* serves as 1-element array */
    mstate ms = (mstate)msp;
    if (!ok_magic(ms))
...
void** cfl_mspace_independent_comalloc(mspace msp, size_t n_elements,
...
int cfl_mspace_trim(mspace msp, size_t pad)
{
    int result = 0;
    mstate ms = (mstate)msp;
    if (ok_magic(ms))
...
void cfl_mspace_malloc_stats(mspace msp)
{
    mstate ms = (mstate)msp;
    if (ok_magic(ms))
    {
...
size_t cfl_mspace_footprint(mspace msp)
{
    size_t result = 0;
    mstate ms = (mstate)msp;
    if (ok_magic(ms))
...
size_t cfl_mspace_max_footprint(mspace msp)
{  /*lint -esym(765,mspace_max_footprint) */
    size_t result = 0;
    mstate ms = (mstate)msp;
    if (ok_magic(ms))
    {
...
#if !NO_MALLINFO
struct mallinfo cfl_mspace_mallinfo(mspace msp)
{
    mstate ms = (mstate)msp;
    if (!ok_magic(ms))
    {
...
#endif /* NO_MALLINFO */
size_t cfl_mspace_usable_size(void* mem)
{
    if (mem != 0)
    {
        mchunkptr p = mem2chunk(mem);
...
int cfl_mspace_mallopt(int param_number, int value) {
  return change_mparam(param_number, value);}
#endif /* MSPACES */
