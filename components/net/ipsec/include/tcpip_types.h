...
#define TCPIP_NETID_INVALIDE (0xFFFFFFFF)
...
typedef struct
{
    ...
    uint16 reserve;
    int32 addr4_num;
    uint32 target_addr4[5];
    int32 addr6_num;
    uint8 target_addr6[5][16];
} TCPIP_DNS_SRV_T;
...
//         when low bearer send packet over, it should called TCPIP_TxCnf to notify
//         TCPIP release the packet buffer.
typedef int32 (*TCPIP_TX_REQ_FPTR_EX)(
    const TCPIP_PACKET_INFO_T *pkt_info_ptr, uint32 pkt_cnt);
...
//WIFI set multicast callback function
typedef int32 (*TCPIP_SET_MCAST_FPTR)(
    TCPIP_NETID_T netid, /* net interface ID */
    uint8 *mc_mac,       /* mac multicast address pointer */
    uint8 num);          /* number of mac multicast address number */
...
typedef struct raw_tx_tag
{
    ...
    TCPIP_TX_REQ_FPTR_EX tx_fptr_ex; /* tx function pointer */
    uint32 link_handle;
} TCPIP_RAW_TX_T;
...
typedef struct netif_cfg_tag
{
    ...
    TCPIP_TX_REQ_FPTR_EX tx_req_fptr_ex; /* tx function pointer for data sending from tcpip to low layer */
    TCPIP_SET_MCAST_FPTR set_mcast_fptr; /* set mac multicast addr function pointer */
    ...
    BOOLEAN act_as_host;
    BOOLEAN enable_boadcast; /*Borad cast*/
    BOOLEAN ndis_mode;       /*TRUE - NDIS*/
    BOOLEAN bypass_mode;     /*TRUE - Bypass*/
} TCPIP_NETIF_CFG_T;
...
typedef struct dhcp_server_config
{
    ...
    uint32 task_id;             /* task to receive result message */
...
typedef enum
{
    TCPIP_FILTER_OPERATE_ACCEPT = 0, /* accept packet */
    TCPIP_FILTER_OPERATE_DROP,       /* drop packet */
    TCPIP_FILTER_OPERATE_LIMIT,
    TCPIP_FILTER_OPERATE_MAX
} TCPIP_Filter_Operate_E;

typedef enum
{
    TCPIP_FILTER_CHAIN_RX = 0,
    TCPIP_FILTER_CHAIN_INPUT,
    TCPIP_FILTER_CHAIN_OUTPUT,
    TCPIP_FILTER_CHAIN_FORWARD,
    TCPIP_FILTER_CHAIN_TX,
    TCPIP_FILTER_CHAIN_MAX
} TCPIP_Filter_Chain_E;

typedef enum
{
    TCPIP_LIMIT_TCP_SYN = 0,
    TCPIP_LIMIT_UDP,
    TCPIP_LIMIT_ICMP_PING, //ping request.
    TCPIP_LIMIT_MAX,
} TCPIP_LIMIT_TYPE;

typedef enum
{
    TCPIP_NONE_RULE = 0, //No specific rule
    TCPIP_LIMIT_RULE,
    TCPIP_RULE_MAX,
} TCPIP_RULE_TYPE;
...
{
    TCPIP_RULE_OK = 0,
    TCPIP_RULE_FAMILY_ERROR,
    TCPIP_RULE_LIMIT_TYPE_ERROR,
    TCPIP_RULE_LIMIT_TIME_ERROR,
    TCPIP_RULE_LIMIT_CHAIN_ERROR,
    TCPIP_RULE_ADD_FILTER_ERROR,
    TCPIP_RULE_DEL_FILTER_ERROR,
} TCPIP_RULE_RETURE;

#ifdef SUPPORT_DOS_DEFENSE
typedef struct TCPIP_Filter_Rule
{
    unsigned long rule_type;
    void *rule_spec;
} TCPIP_Filter_Rule;

typedef struct TCPIP_Limit_Rule
{
    TCPIP_LIMIT_TYPE limit_type;
    unsigned long last;
    unsigned long token_unit;
    unsigned long time_period; //time period
    unsigned long rate;
    unsigned long tokens;
    unsigned long burst;
    unsigned long tokens_max;
} TCPIP_Limit_Rule;
#endif
