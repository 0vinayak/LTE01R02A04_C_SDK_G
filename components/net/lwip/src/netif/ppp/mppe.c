...
mppe_init(ppp_pcb *pcb, ppp_mppe_state *state, u8_t options)
{
	...
	if (options & MPPE_OPT_128)
		...
		LWIP_DEBUGFS(LOG_DEBUG, (OSI_LOGPAR_SI, 0x1000977b, "%s[%d]: unknown key length\n", debugstr,
			pcb->netif->num));
...
#if PPP_DEBUG
	{
		...
		LWIP_DEBUGFS(LOG_DEBUG, (OSI_LOGPAR_SIIS, 0x1000977c, "%s[%d]: initialized with %d-bit %s mode\n",
		...
		LWIP_DEBUGFS(LOG_DEBUG, (OSI_LOGPAR_SISS, 0x1000977d, "%s[%d]: keys: master: %s initial session: %s\n",
...
mppe_decompress(ppp_pcb *pcb, ppp_mppe_state *state, struct pbuf **pb)
{
	...
	if (!state->stateful && !flushed) {
		LWIP_DEBUGF(LOG_DEBUG, (0x10007848, "mppe_decompress[%d]: FLUSHED bit not set in "
		       "stateless mode!\n", pcb->netif->num));
		state->sanity_errors += 100;
		goto sanity_error;
	}
