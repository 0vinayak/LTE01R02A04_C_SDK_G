...
struct mbedtls_ssl_config
{
    ...
    unsigned int ignore_cert_flag; /*!< QUEC adds 
...
#if defined(CONFIG_QUEC_PROJECT_FEATURE_SSL)
/**
 * \brief                  Whether to ignore configuration items
 *                         for checking the counterpart certificate during SSL communication.
 *
 * \note                   quec adds configuration items, not the original mbed function.
 *
 * \param ignore_cert_cfg  SSL ignore certificate checking for configuration items.
 */
void quec_mbedtls_ssl_set_ignore_cert_flag(mbedtls_ssl_config *conf, unsigned int ignore_cert_flag);
#endif
