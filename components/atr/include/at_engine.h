...
//21220122 kevin.wang move it to quec_sio_adapter.h
...
#endif /* end CONFIG_QUEC_PROJECT_FEATURE_ATC_PARSE */
 ...
 *                  this /p cfg can be change by the function,
 *                  invalid items will be corrected to bestfit
 * @return
 ...
 * @param cmuxParent    parent cmux mode engine
...
#endif
/**
 * @brief set comman engine urc enable or disable
 *
 * When disabled, all urc will be ignored for the specified command engine.
 * This is a non-standard feature. By default, urc is enabled.
 *
 * @param th        command mode engine, must be valid
 * @param enable    true for enable urc, false for disable
 */
void atCmdSetUrcEnable(atCmdEngine_t *th, bool enable);

/**
 * @brief whether urc is enabled for command engine
 *
 * @param th        command mode engine, must be valid
 * @return
 *      - true if urc is enabled for the command engine
 *      - flase if urc is disabled
 */
bool atCmdIsUrcEnabled(atCmdEngine_t *th);
 ...
 * @param data      data pointer to be write, can't be NULL is size is not 0
 * @param size      data size
 * @param dlci      DLC number
 */
void atCmuxDlcWrite(atCmuxEngine_t *th, const void *data, size_t size, int dlci);
...
/**
 * @brief Send CTRL msg MSC
 *
 * @param th        cmux mode engine, must be valid
 * @param cr        1:ctrl command, 0:ctrl response
 * @param dlci      virtual circuit path
 * @param v24      v.24 control signal
 * @param br        default 0
 */
void atCmuxSendCtrlMSC(atCmuxEngine_t *th, uint8_t cr, uint8_t dlci, uint8_t v24, uint8_t br);
...
/**
 * @brief User customize the baud rate
 *
 * @param th        AT device pointer, must be valid
 * @return
 *      NONE
 */
void atCmdDeviceEnableUsrBaud(atCmdEngine_t *th);

/**
 * @brief User customize the icf config
 *
 * @param th        AT device pointer, must be valid
 * @return
 *      NONE
 */
void atCmdDeviceSetUsrFormatNeeded(atCmdEngine_t *th);

/**
 * @brief get sim id from the dispatch
 *
 * @param dispatch        the dispatch pointer, must be valid
 * @return      sim id
 */
uint8_t atDispatchGetSim(atDispatch_t *dispatch);

/**
 * @brief get Dlci from the dispatch
 *
 * @param dispatch        the dispatch pointer, must be valid
 * @return      sim id
 */
int atDispatchGetDlci(atDispatch_t *dispatch);

/**
 * @brief get the dispatch from cmux of special sim id and dlci
 *
 * @param nSim sim id
 *        nDlci dlci
 * @return dispatch
 */
atDispatch_t *atCmdGetCmuxDispatch(uint8_t nSim, uint8_t nDlci);
