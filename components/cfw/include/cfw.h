...
#if !defined(CONFIG_SOC_8811) && !defined(CONFIG_SOC_8821)
extern int32_t sys_nst_config(CFW_NW_NST_TX_PARA_CONFIG *nst_para_config);
extern int32_t sys_nst_getrssi(void);
extern int32_t sys_start_nst_mode(void);
extern int32_t sys_stop_nst_mode(void);
#endif
...
uint32_t CFW_SimUpdateBinaryV2(
    uint8_t nFileId,
    uint16_t nOffset,
    uint8_t *pData,
    uint8_t nBytesToWrite,
...
uint32_t CFW_SimGetOperatorNameV2(
    uint8_t pOperatorId[6],
    uint16_t nUTI,
    CFW_SIM_ID nSimID);
//SIM Switch
...
uint8_t CFW_GetSimSwitchIndex(void);

void CFW_SetSimSwitchIndex(uint8_t nIndex);
...
/*! \brief function After opening t3245, check how long
       there is left for t3245 to timeout.
\return \a RRC release value
*/
uint32_t CFW_GetT3245(uint16_t nUTI, CFW_SIM_ID nSimID);
...
uint32_t CFW_SetMeaReportFlag(uint32_t flag, CFW_SIM_ID nSimID);
uint32_t CFW_GetMeaReportFlag(CFW_SIM_ID nSimID);
...
uint32_t CFW_NwGetSinr(
    uint16_t nUTI,
    CFW_SIM_ID nSimID);

uint32_t CFW_NwSetGsmTx(
    uint8_t bActive,
    CFW_SIM_ID nSimID);
...
uint32_t CFW_NwSetMaxAttFailNum(
    uint8_t nFailNum,
    CFW_SIM_ID nSimID);
...
typedef struct
{
    ...
    uint8_t PcscfType[MAX_PCSCF_ADDR_NUMBER];
...
uint32_t CFW_ImsSmscSet(uint8_t *smsc, CFW_SIM_ID nSimID);
...
uint32_t CFW_ImsDisableSmsOverIms(uint8_t bDisable, CFW_SIM_ID nSimID);
uint32_t CFW_ImsDisableEmgReg(uint8_t bDisable, CFW_SIM_ID nSimID);
uint32_t CFW_ImsSetReactPdnWhenLost(uint8_t bEnable, CFW_SIM_ID nSimID);
uint32_t CFW_ImsGetReactPdnWhenLost(uint8_t *pEnable, CFW_SIM_ID nSimID);
uint32_t CFW_CcSetDisableCsfb(uint8_t bDisable, CFW_SIM_ID nSimID);
uint32_t CFW_CcGetDisableCsfb(uint8_t *pDisable, CFW_SIM_ID nSimID);
#define CFW_IMS_MAX_FORBIDDEN_CODE_NUMBER 5
uint32_t CFW_ImsSetForbiddenCode(uint16_t Codes[CFW_IMS_MAX_FORBIDDEN_CODE_NUMBER], CFW_SIM_ID nSimID);
uint32_t CFW_ImsSetKeepPdp(uint8_t bKeep);
uint32_t CFW_ImsSetReAttachForNewPcscf(uint8_t bReAttach, CFW_SIM_ID nSimID);
uint32_t CFW_ImsGetReAttachForNewPcscf(uint8_t *pReAttach, CFW_SIM_ID nSimID);
uint32_t CFW_ImsSetNewPcscfByTimer(uint8_t bByTimer, CFW_SIM_ID nSimID);
uint32_t CFW_ImsGetNewPcscfByTimer(uint8_t *pByTimer, CFW_SIM_ID nSimID);
uint32_t CFW_ImsConfigUserPcscfAddr(
    CFW_USER_PCSCF_ADDR sUserPcscfAddr,
    CFW_SIM_ID nSimID);

uint32_t CFW_CcSetUrn(
    uint8_t category,
    char *pUrn,
    uint8_t nLen);

uint32_t CFW_CcRleaseBearAfterEmc(
    uint8_t bRelease);
//#endif //quectel modify
...
uint32_t CFW_NwSetEsmInfoTransFlag(uint8_t bFlag, CFW_SIM_ID nSimID);
uint32_t CFW_NwGetDrxCycleLen(uint8_t *pLen, CFW_SIM_ID nSimID);
uint32_t CFW_NwSetDrxCycleLen(uint8_t nLen, CFW_SIM_ID nSimID);
uint32_t CFW_NwSetRachFailBar(uint8_t nFailNum, uint8_t nBarTime, CFW_SIM_ID nSimID);
uint32_t CFW_NwGetRachFailBar(uint8_t *pFailNum, uint8_t *pBarTime, CFW_SIM_ID nSimID);
uint32_t CFW_NwGetSupportRat(uint32_t *pRat, CFW_SIM_ID nSimID);
uint8_t CFW_nvGetEdrxValue(uint8_t nSimID);

#define CFW_PREFER_PLMN_LIST_NUM 5
typedef struct
{
    uint8_t prePlmnNum;
    uint8_t prePlmnList[CFW_PREFER_PLMN_LIST_NUM * 6];
} CFW_PREFER_PLMN_LIST;
uint32_t CFW_NwSetPrePlmnList(CFW_PREFER_PLMN_LIST *pPlmnList, CFW_SIM_ID nSimID);
...
uint32_t CFW_NwGetEpsLoci(CFW_EpsLoci *pEpsLoci, CFW_SIM_ID nSimID);
...
typedef enum
{
    ...
    CFW_INIT_STATE = 0xff  ///didn't attach and detach
...
uint32_t CFW_CcIsCsCallActivate(CFW_SIM_ID nSimId);
...
#define CFW_STY_AUTH_PERMANENT_LOCK 27
#define CFW_STY_AUTH_PH_INTEGRITY_FAIL 28
...
uint32_t CFW_CfgSetCombineDetachFlag(uint8_t bFlag, CFW_SIM_ID nSimID);
uint8_t CFW_CfgGetCombineDetachFlag(CFW_SIM_ID nSimID);
uint32_t CFW_CfgSetUseRplmnRatFlag(uint8_t bFlag, CFW_SIM_ID nSimID);
uint8_t CFW_CfgGetUseRplmnRatFlag(CFW_SIM_ID nSimID);
...
#define RF_LTE_NV_BAND_NUM (24)
typedef struct rf_lte_band_calib_info_tag
{
    uint16_t band[RF_LTE_NV_BAND_NUM];
    uint16_t additive[RF_LTE_NV_BAND_NUM];
    uint16_t calibinfo[RF_LTE_NV_BAND_NUM];
} rf_lte_band_calib_info_t;
extern int32_t rf_getlte_calibbandinfo(rf_lte_band_calib_info_t *stp_band_calib_info);

#define LTE_NV_BAND_NUM 24
typedef struct
{
    uint16_t band[LTE_NV_BAND_NUM];
    uint16_t additive[LTE_NV_BAND_NUM];
    uint16_t calibinfo[LTE_NV_BAND_NUM];
} CFW_LTE_BAND_CALIB_INFO_T;
uint32_t CFW_EmodGetLteBandCalibInfo(CFW_LTE_BAND_CALIB_INFO_T *pBandClibInfo);
...
uint32_t CFW_SetTriBandSearchSupport(uint8_t TriBandSearchSupport, CFW_SIM_ID nSimID);
uint8_t CFW_GetTriBandSearchSupport(CFW_SIM_ID nSimID);
...
uint32_t CFW_GprsGetEdrxAllowed(uint8_t *pAllowed, CFW_SIM_ID nSimID);
...
uint32_t CFW_GprsSetGprsClass(uint8_t nClass);
uint32_t CFW_GprsGetGprsClass(uint8_t *pClass);
...
#define CFW_INIT_STATUS_PROVIDER_ID (0x03)
...
uint32_t CFW_EmodSupportDisableIdLte(
    uint8_t bSupport,
    CFW_SIM_ID nSimId);
uint32_t CFW_EmodSetDisableUpdateCsDomainPara(
    uint8_t bDisable,
    CFW_SIM_ID nSimId);
uint8_t CFW_EmodGetDisableUpdateCsDomainPara(
    CFW_SIM_ID nSimId);
...
uint32_t CFW_EmodSetDisableForbidHplmn(
    uint8_t bDisable,
    CFW_SIM_ID nSimID);

uint32_t CFW_EmodGetDisableForbidHplmn(
    uint8_t *bDisable,
    CFW_SIM_ID nSimID);
...
typedef struct _CFW_TSM_CURR_CELL_INFO
{
    ...
    uint8_t nTSM_CellID[4]; //Cell Identity(Gsm only uses the first two bytes).
...
uint32_t CFW_SetRFFreqPwrRange_V2(uint16_t mode, uint16_t band, int16_t powerlow, int16_t powerhigh);
uint32_t CFW_GetRFFreqPwrRange_V2(uint16_t mode, uint16_t band, int16_t *powerlow, int16_t *powerhigh);
...
typedef struct T_RADIO_STATS_RPT_TAG
{
    ...
    uint32_t last_earfcn;       //<last earfcn value>(change u16 to u32 to support band255 band256)
...
{
    uint32_t earfcn;      //<earfcn>           absolute radio-frequency channel number
...
uint32_t CFW_GetNvUpdateFlag(uint32_t *nFlag, CFW_SIM_ID nSimID);

typedef enum
{
    NV_NW_SIM_PLMN = 0,
    NV_NW_SIM_BA_COUNT,
    NV_NW_SIM_BA_COUNT_PENDING,
    NV_NW_SIM_ARFCE_LIST,
    NV_NW_SIM_STORE_LAST,
    NV_SMS_SIM_PARAM_MR,
    NV_GPRS_SIM_DOWN_SUM,
    NV_GPRS_SIM_UP_SUM,
    NV_SMS_SIM_PARAM,
} CFW_NV_UPDATE_TYPE;
...
typedef struct
{
    uint8_t type_of_shape;
    //TODO
    //need for future modification
    //currently not support add-geographicalInformation
...
/******************************************************************
** Description: configure RPLMN
** Parameters:
**   pRplmnInfo: rplmn info
**   nSimID: sim index
** return: 0 success, others error
******************************************************************/
typedef struct
{
    uint8_t mode; //0:enable, others:disable
    uint8_t rplmn[6];
    uint8_t rplmnLen;
    uint8_t rat; //0: auto, 2:gsm, 4:lte
} CFW_RPLMN_INFO;
uint32_t CFW_NwSetRplmn(CFW_RPLMN_INFO pRplmnInfo, CFW_SIM_ID nSimID);
...
void CFW_SimlockReadConfigImei(uint8_t pImei[16], CFW_SIM_ID nSimID);
#endif

uint32_t CFW_SetMoSmsRetryCount(uint8_t smc_count, uint8_t smr_count, CFW_SIM_ID nSimID);
uint32_t CFW_NwSetWeakSignalEnable(uint8_t nMode, CFW_SIM_ID nSimID);
uint32_t CFW_NwGetWeakSignalEnable(CFW_SIM_ID nSimID);

#define CFW_MAX_BAND_PWOER_LEN 30
typedef struct _CFW_POWER_BOOST
{
    uint8_t powerboost_db1[16];
    uint8_t powerboost_db2[14];
} CFW_POWER_BOOST;
uint32_t CFW_NwSetBandPowerBoost(CFW_POWER_BOOST powerboost, CFW_SIM_ID nSimID);
...
#if defined(CONFIG_SOC_8821)
typedef enum _CFW_NBIOT_BAND_TABLE
{
    CFW_NBIOT_BAND_255 = 0,
    CFW_NBIOT_BAND_256 = 1
} CFW_NBIOT_BAND_TABLE;
#endif
...
uint32_t CFW_CellReselectOffset(CFW_CELL_RESELECT_T cellReselect, CFW_SIM_ID nSim);
typedef struct
{
...
uint32_t CFW_NwSetStackCfgGea2(uint8_t gea1, uint8_t gea2);
uint8_t CFW_NwGetStackCfgGea2();

extern uint8_t tgl00_16NetworkTestFalg(uint32_t simFlag);
extern uint8_t tgl00_23OperatorTestNwFlag(uint32_t simFlag);
...
{
    uint8_t cid;
} CFW_GPRS_ACTRESULT_T;

typedef struct
{
    uint8_t PD; // MM_MMC_PD  CMN_MMP_PD
    uint8_t msg;
    uint8_t cause;
    uint8_t Bsic;
    uint16_t Arfcn;
    uint8_t Rai[6];
} CFW_GSM_EVENT_REJ_CAUSE_T;
/**
 * @brief get the configured sim number of ps.
 *
 * @param : void.
 *
 * @return 2 or 1 according to target config
 */
uint8_t CFW_GeneralGetPsSimNumber(void);

/**
 * @brief get the nvm static pointer by sim id.
 *
 * @param : sim index.
 *
 * @return the nvm static pointer by sim id
 */
void *CFW_GeneralGetNvmStatic(CFW_SIM_ID nSimId);
uint8_t CFW_GeneralGetCsmMailboxId(void);

uint32_t CFW_SimSetAttribute(uint32_t attribute, CFW_SIM_ID nSimID);
uint32_t CFW_SimGetAttribute(CFW_SIM_ID nSimID);

#ifdef CONFIG_FUNC_SUPPORT_SIMLOCK_AP
typedef enum
{
    CFW_SIMLOCK_OK = 0,
    CFW_SIMLOCK_LOCK_NET,
    CFW_SIMLOCK_PERMANENT_LOCK,
    CFW_SIMLOCK_INTEGRITY_FAIL
} CFW_SIMLOCK_ERROR_TYPE;
#endif
...
/*********************************************************************
** Description: CFW_TestCardAutoPdnControl
** parameters:
** nSupport: range(0,1);0: default,enable auto pdn activate; 1: disable auto pdn activate
** nSimID: sim index
** return: success, fail(0x04)
**********************************************************************/
uint32_t CFW_TestCardAutoPdnControl(uint8_t nSupport, CFW_SIM_ID nSimId);

void *CFW_GeneralGetPsmCard2Info(void);
