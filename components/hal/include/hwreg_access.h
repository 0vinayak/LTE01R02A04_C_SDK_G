...
// Mask for [hi:lo]. REG_BIT(hi + 1) will cause -Wshift-count-overflow
#define REG_BIT_RANGE_MASK(hi, lo) ({ unsigned _h = (hi) + 1; unsigned _hm = (_h >= 32)? 0 : REG_BIT(_h); _hm - REG_BIT(lo); })
...
#define HAL_CP_ICIALLU 15, c7, 0, c5, 0   // Instruction Cache Invalidate All to PoU
#define HAL_CP_ICIMVAU 15, c7, 0, c5, 1   // Instruction Cache Invalidate by MVA to PoU
#define HAL_CP_DCIMVAC 15, c7, 0, c6, 1   // Data Cache Invalidate by MVA to PoC
#define HAL_CP_DCISW 15, c7, 0, c6, 2     // Data Cache Invalidate by Set/Way
#define HAL_CP_DCCMVAC 15, c7, 0, c10, 1  // Data Cache Clean by MVA to PoC
#define HAL_CP_DCCSW 15, c7, 0, c10, 2    // Data Cache Clean by Set/Way
#define HAL_CP_DCCIMVAC 15, c7, 0, c14, 1 // Data Cache Clean and Invalidate by MVA to PoC
#define HAL_CP_DCCISW 15, c7, 0, c14, 2   // Data Cache Clean and Invalidate by Set/Way
...
/**
 * \brief register value by fields
 */
#define REGT_VAL(type, ...) __REGT_VAL(type, ##__VA_ARGS__)

/**
 * \brief pair of (mask, value)
 */
#define REGT_MASKVAL(type, ...) __REGT_MASK(type, ##__VA_ARGS__), __REGT_VAL(type, ##__VA_ARGS__)
...
#define __REGT_MASK(type, ...) ({ type _val = {0xffffffff}; __REGV_MASKOFF(_val, __VA_ARGS__); ~_val.v; })
#define __REGT_VAL(type, ...) ({ type _val = {0}; __REGV_SET(_val, __VA_ARGS__); _val.v; })
