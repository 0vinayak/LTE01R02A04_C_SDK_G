...
#include "Core_A/cmsis_compiler.h"               /* CMSIS compiler specific defines */
...
/** \brief Enable I Caches by setting I bit in SCTLR register.
*/
__STATIC_FORCEINLINE void L1C_EnableICaches(void) {
  __set_SCTLR( __get_SCTLR() | SCTLR_I_Msk );
  __ISB();
}

/** \brief Disable I Caches by clearing I bits in SCTLR register.
*/
__STATIC_FORCEINLINE void L1C_DisableICaches(void) {
  __set_SCTLR( __get_SCTLR() & (~SCTLR_I_Msk));
  __ISB();
}
#endif
...
//Sect_No_Access
#define region_no_access(region, type)     region.rg_t = type; \
                                   region.domain = 0x0; \
...
#define region_normal_nc(region, type)     region.rg_t = type; \
                                   region.domain = 0x0; \
                                   region.e_t = ECC_DISABLED; \
                                   region.g_t = GLOBAL; \
                                   region.inner_norm_t = NON_CACHEABLE; \
...
#define region_normal_cod(region, type) region.rg_t = type; \
                                   region.domain = 0x0; \
                                   region.e_t = ECC_DISABLED; \
                                   region.g_t = GLOBAL; \
                                   region.inner_norm_t = WB_WA; \
...
#define region_normal_ro(region, type)  region.rg_t = type; \
                                   region.domain = 0x0; \
                                   region.e_t = ECC_DISABLED; \
                                   region.g_t = GLOBAL; \
                                   region.inner_norm_t = WB_WA; \
...
#define region_normal_rw(region, type) region.rg_t = type; \
                                   region.domain = 0x0; \
                                   region.e_t = ECC_DISABLED; \
                                   region.g_t = GLOBAL; \
                                   region.inner_norm_t = WB_WA; \
...
#define region_so(region, type) region.rg_t = type; \
                                   region.domain = 0x0; \
                                   region.e_t = ECC_DISABLED; \
                                   region.g_t = GLOBAL; \
                                   region.inner_norm_t = NON_CACHEABLE; \
...
#define region_device_ro(region, type) region.rg_t = type; \
                                   region.domain = 0x0; \
                                   region.e_t = ECC_DISABLED; \
                                   region.g_t = GLOBAL; \
                                   region.inner_norm_t = NON_CACHEABLE; \
...
#define region_device_rw(region, type) region.rg_t = type; \
                                   region.domain = 0x0; \
                                   region.e_t = ECC_DISABLED; \
                                   region.g_t = GLOBAL; \
                                   region.inner_norm_t = NON_CACHEABLE; \
...
//Page_64k_Normal. Outer & inner wb/wa, non-shareable, executable, rw, domain 0
#define page64k_normal(descriptor_l1, descriptor_l2, region)  region.rg_t = PAGE_64k; \
                                   region.domain = 0x0; \
                                   region.e_t = ECC_DISABLED; \
                                   region.g_t = GLOBAL; \
                                   region.inner_norm_t = WB_WA; \
...
__STATIC_INLINE void MMU_Enable(void)
{
  ...
  //__set_SCTLR( (__get_SCTLR() & ~(1 << 28) & ~(1 << 1)) | 1);
  __ISB();
}
