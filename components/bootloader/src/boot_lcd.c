...
OSI_WEAK const drvLcdPanelDesc_t *drvLcdGetDesc(void *inst)
{
    bootLcd_t *d = (bootLcd_t *)inst;
    return d->desc;
}

OSI_WEAK void drvLcdWriteCmd(void *inst, uint16_t cmd)
{
    bootLcd_t *d = (bootLcd_t *)inst;
...
OSI_WEAK void drvLcdWriteData(void *inst, uint16_t data)
{
    bootLcd_t *d = (bootLcd_t *)inst;
    OSI_LOOP_WAIT((hwp_gouda->gd_status & GOUDA_LCD_BUSY) == 0);
    hwp_gouda->gd_lcd_ctrl = d->lcd_ctrl.v;
...
OSI_WEAK void drvLcdReadData(void *inst, uint16_t cmd, uint8_t *data, unsigned len)
{
    bootLcd_t *d = (bootLcd_t *)inst;
