 ...
 *      - -1 on error. Refer to file system implementation for errno.
 */
int vfs_create(const char *path, int flags, size_t size);
#endif
 ...
 * helper to get file size
 *
 * It calls \p vfs_fstat to get file status, and return file size only.
 *
 * \param [in] fd     file descriptor
 * \return
 *      - file size on success
...
int prvunicode2utf8(unsigned char *src, int slen, char *dst, int dlen);

int prvutf82unicode(char *src, char *dst, int dlen);
#endif

typedef struct
{
    ssize_t (*stdin_read)(void *dst, size_t size);
    ssize_t (*stdout_write)(const void *data, size_t size);
    ssize_t (*stderr_write)(const void *data, size_t size);
} vfs_stdio_callback_t;
