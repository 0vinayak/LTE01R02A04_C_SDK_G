...
#include "ql_gprs_data_transfer.h"

#define QL_DATACALL_LOG_LEVEL	QL_LOG_LEVEL_INFO
#define QL_TRANSFER_DEMO_LOG(msg, ...)			QL_LOG(QL_DATACALL_LOG_LEVEL, "ql_gprs_data_transfer", msg, ##__VA_ARGS__)
#define QL_SIM_NUM				  2
...
ql_sem_t ql_data_reg_sem[QL_SIM_NUM] = {0};
...
void ql_nw_ind_callback(uint8_t sim_id, unsigned int ind_type, void *ind_msg_buf)
{
    if(QUEC_NW_DATA_REG_STATUS_IND == ind_type)
    {
        ...
        QL_TRANSFER_DEMO_LOG("Sim%d data reg status changed, current status is %d", sim_id, data_reg_status->state);
...
{
    QL_TRANSFER_DEMO_LOG("nSim = %d, profile_idx=%d, ind_type=0x%x, result=%d", sim_id, profile_idx, ind_type, result);
...
void data_transfer_tcpip_input(void *playlod, unsigned short tot_len)
{
	QL_TRANSFER_DEMO_LOG("tot_len=%d",tot_len);
	QL_TRANSFER_DEMO_LOG("not free playlod!!!!!!!");
}

static void gprs_data_transfer_app_thread(void * arg)
{
    ...
    if(0 != ret)
    {
        QL_TRANSFER_DEMO_LOG("ql_nw_register_cb err, ret=0x%x", ret);
        goto exit;
    }

    ret = ql_nw_get_reg_status(sim_id, &nw_info);
    QL_TRANSFER_DEMO_LOG("ret: 0x%x, current data reg status=%d", ret, nw_info.data_reg.state);
    ...
    QL_TRANSFER_DEMO_LOG("ql_datacall_register_cb ret=0x%x", ret);
    ...
    if(0 != ret)
    {
        QL_TRANSFER_DEMO_LOG("ql_start_data_call err, ret=0x%x", ret);
        goto exit;
    }

    ql_rtos_semaphore_wait(datacall_ctx[sim_id][profile_idx-1].act_sem, QL_WAIT_FOREVER);  //wait QUEC_DATACALL_ACT_RSP_IND
    QL_TRANSFER_DEMO_LOG("datacall state=%d", datacall_ctx[sim_id][profile_idx-1].state);

    ret = ql_get_data_call_info(sim_id, profile_idx, &info);
    QL_TRANSFER_DEMO_LOG("ql_get_data_call_info ret: 0x%x", ret);
    QL_TRANSFER_DEMO_LOG("info.profile_idx: %d, info.ip_version: %d", info.profile_idx, info.ip_version);
	QL_TRANSFER_DEMO_LOG("info->v4.state: %d, info.v6.state: %d", info.v4.state, info.v6.state);
    if(info.v4.state)
    {
		QL_TRANSFER_DEMO_LOG("info.v4.addr.ip: %s", ip4addr_ntoa(&(info.v4.addr.ip)));
		QL_TRANSFER_DEMO_LOG("info.v4.addr.pri_dns: %s", ip4addr_ntoa(&(info.v4.addr.pri_dns)));
		QL_TRANSFER_DEMO_LOG("info.v4.addr.sec_dns: %s", ip4addr_ntoa(&(info.v4.addr.sec_dns)));
    }
    if(info.v6.state)
    {
		QL_TRANSFER_DEMO_LOG("info.v6.addr.ip: %s", ip6addr_ntoa(&(info.v6.addr.ip)));
		QL_TRANSFER_DEMO_LOG("info.v6.addr.pri_dns: %s", ip6addr_ntoa(&(info.v6.addr.pri_dns)));
		QL_TRANSFER_DEMO_LOG("info.v6.addr.sec_dns: %s", ip6addr_ntoa(&(info.v6.addr.sec_dns)));
    }


	ql_gprs_data_transfer_input_cb_reg(data_transfer_tcpip_input);
    ret = ql_gprs_data_transfer_start(sim_id, profile_idx);
	if(ret == TRUE)
	{
		//ql_gprs_data_transfer_output("ip type data", strlen("ip type data"));
	}
	//ql_gprs_data_transfer_stop();
	...
	while(1)
	{
        ...
        {
        	ql_gprs_data_transfer_stop();  //stop gprs_data_transfer
            ...
            {
            	retry_count++;
                QL_TRANSFER_DEMO_LOG("gprs reconnect fail, the retry count is %d", retry_count);
            ...
            if(QL_DATACALL_SUCCESS == ret)
            {
                ...
                ql_gprs_data_transfer_input_cb_reg(data_transfer_tcpip_input);
    			ql_gprs_data_transfer_start(sim_id, profile_idx);
            }
        }
	}
    
exit:
    ql_rtos_semaphore_delete(ql_data_reg_sem[sim_id]);
...
void ql_gprs_data_transfer_app_init(void)
{
    QlOSStatus err = QL_OSI_SUCCESS;
    	
	err = ql_rtos_task_create(&datacall_task, 4*1024, APP_PRIORITY_NORMAL, "QGPRSDATADEMO", gprs_data_transfer_app_thread, NULL, 10);
    if(err != QL_OSI_SUCCESS)
	{
		QL_TRANSFER_DEMO_LOG("task created failed");
        return;
	}
