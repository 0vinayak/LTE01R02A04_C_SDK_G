...
if(CONFIG_QUEC_PROJECT_FEATURE_PWM_AUDIO)
option(QL_APP_FEATURE_PWM_AUDIO  "Enable PWM_AUDIO" ON)
else()
message(STATUS "FEATURE PWM_AUDIO is disabled at core!")
option(QL_APP_FEATURE_PWM_AUDIO  "Enable PWM_AUDIO" OFF)
endif()
message(STATUS "QL_APP_FEATURE_PWM_AUDIO ${QL_APP_FEATURE_PWM_AUDIO}")

if(CONFIG_QUEC_PROJECT_FEATURE_PWM_AUDIO_DEC)
option(QL_APP_FEATURE_PWM_AUDIO_DEC  "Enable PWM AUDIO DEC" ON)
else()
message(STATUS "FEATURE PWM AUDIO DEC is disabled at core!")
option(QL_APP_FEATURE_PWM_AUDIO_DEC  "Enable PWM AUDIO DEC" OFF)
endif()
message(STATUS "QL_APP_FEATURE_PWM_AUDIO_DEC ${QL_APP_FEATURE_PWM_AUDIO_DEC}")

if((QL_APP_FEATURE_AUDIO OR QL_APP_FEATURE_PWM_AUDIO) AND (CONFIG_QUEC_PROJECT_FEATURE_TTS))
...
if(CONFIG_QUEC_PROJECT_FEATURE_LWM2M)
option(QL_APP_FEATURE_LWM2M  "Enable LWM2M" ON)
else()
message(STATUS "FEATURE LWM2M is disabled at core!")
option(QL_APP_FEATURE_LWM2M  "Enable LWM2M" OFF)
endif()
message(STATUS "QL_APP_FEATURE_LWM2M ${QL_APP_FEATURE_LWM2M}")
...
if(CONFIG_QUEC_PROJECT_FEATURE_L2TP)
option(QL_APP_FEATURE_L2TP   "Enable L2TP" ON)
else()
message(STATUS "FEATURE L2TP  is disabled at core!")
option(QL_APP_FEATURE_L2TP   "Enable L2TP" OFF)
endif()
message(STATUS "QL_APP_FEATURE_L2TP  ${QL_APP_FEATURE_L2TP}")

if(CONFIG_QUEC_PROJECT_FEATURE_QUECSCAN)
option(QL_APP_FEATURE_QUECSCAN   "Enable QUECSCAN" ON)
else()
message(STATUS "FEATURE QUECSCAN  is disabled at core!")
option(QL_APP_FEATURE_QUECSCAN   "Enable QUECSCAN" OFF)
endif()
message(STATUS "QL_APP_FEATURE_QUECSCAN  ${QL_APP_FEATURE_QUECSCAN}")

if(CONFIG_QUEC_PROJECT_FEATURE_KWS)
option(QL_APP_FEATURE_KWS   "Enable KWS" ON)
else()
message(STATUS "FEATURE KWS  is disabled at core!")
option(QL_APP_FEATURE_KWS   "Enable KWS" OFF)
endif()
message(STATUS "QL_APP_FEATURE_KWS  ${QL_APP_FEATURE_KWS}")
################################################################################################################
# Quectel open sdk package config
################################################################################################################
if (QL_APP_FEATURE_TTS OR QL_APP_FEATURE_BT OR QL_APP_FEATURE_HD_GNSS)
...
if(QL_APP_PACK_FILE)
    if(QL_APP_FEATURE_TTS)
        if(QL_APP_FEATURE_BT)
            if (CONFIG_QUEC_PROJECT_FEATURE_BT_BLE_PACK EQUAL 2)    # == 2
                if(QL_APP_FEATURE_HD_GNSS)
                    set(QL_APP_PACK_FILE_JSON_PATH components/ql-config/download/prepack/ql_prepack_tts_bt_ble_gps.json)
                else()
                    set(QL_APP_PACK_FILE_JSON_PATH components/ql-config/download/prepack/ql_prepack_tts_bt_ble.json)
                endif()
            else()
                if(QL_APP_FEATURE_HD_GNSS)
                    set(QL_APP_PACK_FILE_JSON_PATH components/ql-config/download/prepack/ql_prepack_tts_ble_gps.json)
                else()
                    set(QL_APP_PACK_FILE_JSON_PATH components/ql-config/download/prepack/ql_prepack_tts_ble.json)
                endif()
            endif()
        else()
            if(QL_APP_FEATURE_HD_GNSS)
        ...
        if(QL_APP_FEATURE_BT)
            if(CONFIG_QUEC_PROJECT_FEATURE_BT_BLE_PACK EQUAL 2)     # == 2
                if(QL_APP_FEATURE_HD_GNSS)
                    set(QL_APP_PACK_FILE_JSON_PATH components/ql-config/download/prepack/ql_prepack_bt_ble_gps.json)
                else()
                    set(QL_APP_PACK_FILE_JSON_PATH components/ql-config/download/prepack/ql_prepack_bt_ble.json)
                endif()
            else()
                if(QL_APP_FEATURE_HD_GNSS)
                    set(QL_APP_PACK_FILE_JSON_PATH components/ql-config/download/prepack/ql_prepack_ble_gps.json)
                else()
                    set(QL_APP_PACK_FILE_JSON_PATH components/ql-config/download/prepack/ql_prepack_ble.json)
                endif()
            endif()
        else()
            if(QL_APP_FEATURE_HD_GNSS)
