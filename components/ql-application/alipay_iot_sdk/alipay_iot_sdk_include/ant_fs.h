 ...
 */
#define ANT_FS_READ               (1<<0)  // 从开始位置读文件
#define ANT_FS_WRITE              (1<<1)  // 将文件截断为0并从开始位置写文件, 如果文件不存在则创建文件
#define ANT_FS_APPEND             (1<<2)  // 从文件末尾追加内容, 如果文件不存在则创建文件
#define ANT_FS_READ_WRITE         (1<<3)  // 从开始位置读写文件, 如果文件不存在应该返回失败
 ...
 *               NULL：获取失败
 */
ant_char *ant_fs_get_root_path(void);
 ...
 *         0： 目录不存在
 */
ant_s32 ant_fs_dir_exist(char *dirname);
 ...
 *          其他值: 获取成功
 */
ant_s32 ant_fs_getc(ant_s32 h);


char *ant_fs_get_real_path(char *filename);


#ifdef __EXT_FLASH_SUPPORT__
