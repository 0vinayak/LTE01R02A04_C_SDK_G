...
/*
* @brief: get sdk build time.
* @output: a char pointer of sdk build time, need not free.
*/
char*  alipay_iot_get_sdk_build_time(void);

typedef void (*alipay_iot_init_callback)(int32_t result);
/**
* @brief: fusion sdk init function.
* @input: (module_supplier_no,device_supplier_no, merchant_user_id, item_id, product_key) profile info. 
* @input: task_priority. task priority who call this function.
* @input: callback.
*/
void alipay_iot_setup(char *module_supplier_no,char *device_supplier_no,char *merchant_user_id,char *item_id,char *product_key,int32_t task_priority, alipay_iot_init_callback callback);
