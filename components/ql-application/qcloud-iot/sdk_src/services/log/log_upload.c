...
int init_log_uploader(LogUploadInitParams *init_params)
{
	...
	pLogClient->http_client->http.profile_idx = init_params->profile_idx;
...
static int _post_one_http_to_server(unsigned char profile_idx, char *post_buf, size_t post_size)
{
    int rc = 0;
    Qcloud_IoT_Log *pLogClient = get_log_client();
	...
	pLogClient->http_client->http.profile_idx = profile_idx;
...
static int post_log_to_server(unsigned char profile_idx, char *post_buf, size_t post_size, size_t *actual_post_payload)
{
#define LOG_DELIMITER "\n\f"
    ...
    if (post_size < MAX_HTTP_LOG_POST_SIZE) {
        ...
        ret = _post_one_http_to_server(profile_idx, post_buf, post_size);
    ...
    *actual_post_payload  = 0;
    ...
    do {
        ...
        ret = _post_one_http_to_server(profile_idx,post_buf, upload_size);
        if (QCLOUD_RET_SUCCESS != ret) {
    ...
    if (whole_log_size > 0) {
        ...
        if (log_buf != NULL) {
            ...
            if (read_len == whole_log_size) {
                ...
                rc = post_log_to_server(pLogClient->http_client->http.profile_idx, log_buf, upload_size, &actual_post_payload);
...
int do_log_upload(bool force_upload)
{
    ...
    size_t actual_post_payload;
    rc = post_log_to_server(pLogClient->http_client->http.profile_idx, pLogClient->log_buffer, upload_log_size, &actual_post_payload);
    if (rc != QCLOUD_RET_SUCCESS) {
