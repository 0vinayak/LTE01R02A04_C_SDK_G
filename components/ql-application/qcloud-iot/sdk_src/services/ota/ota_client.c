...
/* Init OTA handle */
void *IOT_OTA_Init(unsigned char profile_idx, const char *product_id, const char *device_name, void *ch_signal)
{
...
int IOT_OTA_StartDownload(void *handle, uint32_t offset, uint32_t size)
{
    ...
    // reinit ofc
    qcloud_ofc_deinit(h_ota->ch_fetch);
    h_ota->ch_fetch     = ofc_Init(h_ota->profile_idx, h_ota->purl, offset, size);
