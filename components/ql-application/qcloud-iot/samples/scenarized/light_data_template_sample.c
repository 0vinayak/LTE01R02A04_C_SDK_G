...
//    sprintf(sg_key_file, "%s/%s/%s", current_path, certs_dir, sg_devInfo.dev_key_file_name);
    ...
    Log_i("sg_cert_file:%s",sg_cert_file);
    Log_i("sg_key_file:%s",sg_key_file);
...
void light_data_template_sample(void *arg)
{
...
int light_data_template_sample_start(void)
{
    int err = 0;
    int count = 0;
   	ql_task_t light_data_template_sample_task = NULL;
    /* 主循环进入休眠 */
    while (count < 10)
    {
        count++;
        ql_rtos_task_sleep_s(1);
        Log_i("ql_qcloud_sdk_light_data_template_sample_start_init  -0x%04X\n", err);
    }
	err = ql_rtos_task_create(&light_data_template_sample_task, 10*1024, APP_PRIORITY_NORMAL, "light_data", light_data_template_sample, NULL, 5);
	
	if (err != QL_OSI_SUCCESS)
	{
        Log_e("light_data_template sample task created failed");
        return -1;
	}
    return 0;
}
