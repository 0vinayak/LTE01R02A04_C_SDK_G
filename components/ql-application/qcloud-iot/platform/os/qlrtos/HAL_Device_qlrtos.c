...
/* product Id  */
static char sg_product_id[MAX_SIZE_OF_PRODUCT_ID + 1] = "KSS57PUDXP";

/* device name */
static char sg_device_name[MAX_SIZE_OF_DEVICE_NAME + 1] = "Smart_light_03";
...
static char sg_product_secret[MAX_SIZE_OF_PRODUCT_SECRET + 1] = "qatTLzw7493NODafMmNNttoU";
#endif

#ifdef AUTH_MODE_CERT
/* public cert file name of certificate device */
static char sg_device_cert_file_name[MAX_SIZE_OF_DEVICE_CERT_FILE_NAME + 1] = "./certs/Smart_light_03_cert.crt";
/* private key file name of certificate device */
static char sg_device_privatekey_file_name[MAX_SIZE_OF_DEVICE_SECRET_FILE_NAME + 1] = "./certs/Smart_light_03_private.key";
#else
/* device secret of PSK device */
static char sg_device_secret[MAX_SIZE_OF_DEVICE_SECRET + 1] = "HdpsormITQtQvaQiKff5UA==";
#endif
...
int device_info_copy(void *pdst, void *psrc, uint8_t max_len)
{
...
static char sg_device_info_file[MAX_SIZE_OF_DEVICE_INFO_FILE + 1] = {"UFS:device_info.json"};
...
int HAL_GetDevInfoFromFile(const char *file_name, void *dev_info)
{
    ...
    ql_frewind(fp);
    rlen = ql_fread(JsonDoc, 1, len, fp);
...
static int iot_parse_subdevinfo_from_json_file(DeviceInfo *pDevInfo, int *subDevNum)
{
    ...
    fp = ql_fopen(sg_device_info_file, "r");
    if (fp<0) {
...
int HAL_GetGwDevInfoFromFile(const char *file_name, void *pgwDeviceInfo)
{
    ...
    ret = HAL_GetDevInfoFromFile(file_name, &(gwDevInfo->gw_info));
...
// key device info or cert device info
    ...
    fp = ql_fopen(sg_device_info_file, "w");
    if (fp<0) {
        Log_e("open file %s failed", sg_device_info_file);
        return QCLOUD_ERR_FAILURE;
    }

    wlen = ql_fwrite(JsonDoc, 1, strlen(JsonDoc), fp);
...
int HAL_GetDevInfo(void *pdevInfo)
{
    ...
    ret |= device_info_copy(devInfo->product_secret, sg_product_secret,
                            MAX_SIZE_OF_PRODUCT_SECRET);  // get product secret
#endif
