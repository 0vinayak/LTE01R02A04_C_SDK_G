 ...
 */
int _mbedtls_udp_connect(mbedtls_net_context *socket_fd, unsigned char profile_idx, const char *host, int port)
{
    int  ret = 0;
    char port_str[6];
    HAL_Snprintf(port_str, 6, "%d", port);
	#ifdef CONFIG_QUEC_PROJECT_FEATURE
    if ((ret = mbedtls_net_connect(socket_fd, host, port_str, MBEDTLS_NET_PROTO_UDP, profile_idx)) != 0) {
	#else
	if ((ret = mbedtls_net_connect(socket_fd, profile_idx, host, port, MBEDTLS_NET_PROTO_UDP)) != 0) {
	#endif	
...
uintptr_t HAL_DTLS_Connect(DTLSConnectParams *pConnectParams, const char *host, int port)
{
    ...
    if ((ret = _mbedtls_udp_connect(&(pDataParams->socket_fd), pDataParams->profile_idx, host, port)) != QCLOUD_RET_SUCCESS) {
        goto error;
    }
