...
static int _mbedtls_client_init(TLSDataParams *pDataParams, TLSConnectParams *pConnectParams)
{
	...
	pDataParams->profile_idx = pConnectParams->profile_idx;
 ...
 */
int _mbedtls_tcp_connect(mbedtls_net_context *socket_fd, unsigned char profile_idx, const char *host, int port)
{
    int  ret = 0;
	char port_str[6];
    HAL_Snprintf(port_str, 6, "%d", port);
	#ifdef CONFIG_QUEC_PROJECT_FEATURE
    if ((ret = mbedtls_net_connect(socket_fd, host, port_str, MBEDTLS_NET_PROTO_TCP, profile_idx)) != 0) {
	#else
	if ((ret = mbedtls_net_connect(socket_fd, profile_idx, host, port, MBEDTLS_NET_PROTO_TCP)) != 0) {
	#endif	
...
static unsigned int ignoreVerifyFlags = MBEDTLS_X509_BADCERT_EXPIRED | \
...
static int _qcloud_server_certificate_verify(void *hostname, mbedtls_x509_crt *crt, int depth, uint32_t *flags)
{
    ((void) hostname);
    ((void) crt);
	((void) depth);

    Log_w("peer cert verify flags on curt way: 0x%08X", *flags);

    if(*flags & ignoreVerifyFlags)
    {
        *flags &= ~(ignoreVerifyFlags);
        return( 0 );
    }
...
uintptr_t HAL_TLS_Connect(TLSConnectParams *pConnectParams, const char *host, int port)
{
    ...
    if ((ret = _mbedtls_tcp_connect(&(pDataParams->socket_fd), pDataParams->profile_idx, host, port)) != QCLOUD_RET_SUCCESS) {
        goto error;
    }
    ...
    if( ( flags = mbedtls_ssl_get_verify_result(&(pDataParams->ssl))) != 0 )
    {
        char vrfy_buf[512] = {0};
