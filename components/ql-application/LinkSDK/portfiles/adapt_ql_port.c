...
#define QL_ADAPT_LOG_LEVEL	                QL_LOG_LEVEL_INFO
#define QL_ADAPT_LOG(msg, ...)			    QL_LOG(QL_ADAPT_LOG_LEVEL, "ql_Adapt", msg, ##__VA_ARGS__)
...
int32_t core_sysdep_network_setopt(void *handle, core_sysdep_network_option_t option, void *data)
{
    ...
    switch (option) {
		case CORE_SYSDEP_NETWORK_PROFILE_IDX:
			network_handle->profile_idx = *(uint32_t *)data;
		break;
...
static int32_t _core_sysdep_network_connect(int profile_index, char *host, uint16_t port, int family, int socktype, int protocol, uint32_t timeout_ms, int *fd_out)
{
   ...
   int fd = 0, sock_option = 0;
	...
	ql_get_data_call_info(0, profile_index, &info);

    res = getaddrinfo_with_pcid(host, service, &hints, &addrInfoList, profile_index);
	if(res != 0)
	{
        QL_ADAPT_LOG("*** getaddrinfo_with_pcid fail ret =%d ***\r\n",res);
		return res;
	}
    QL_ADAPT_LOG("getaddrinfo_with_pcid success profile_index=%d",profile_index);
    ...
    if (res == 0) 
    {
        for (pos = addrInfoList; pos != NULL; pos = pos->ai_next) {
            ...
            inet_aton(ip4addr_ntoa(&info.v4.addr.ip), &ip4_local_addr.sin_addr);	

			res = bind(fd, (struct sockaddr *)&ip4_local_addr, sizeof(ip4_local_addr));
			if(res < 0)
			{
				return res;
			}
            ...
            } else 
            {
                ...
                if (res == 0) {
                    *fd_out = fd;
                    res = STATE_SUCCESS;
                    QL_ADAPT_LOG("connect success");
                    break;
                } else if (lwip_get_error(fd) != EINPROGRESS) {
                    QL_ADAPT_LOG("connect EINPROGRESS errno =%d",lwip_get_error(fd));
                    res = STATE_PORT_NETWORK_CONNECT_FAILED;
                } else 
                {
                    res = select(fd + 1, &read_sets, &write_sets, NULL, &timeselect);
                    if (res == 0 ) {
                        QL_ADAPT_LOG("connect STATE_MQTT_LOG_CONNECT_TIMEOUT");                        
                        res = STATE_MQTT_LOG_CONNECT_TIMEOUT;
                    } else if (res < 0) {
                        QL_ADAPT_LOG("connect STATE_PORT_NETWORK_CONNECT_FAILED");                        
                        res = STATE_PORT_NETWORK_CONNECT_FAILED;
                    } else {
						if(!FD_ISSET(fd, &read_sets) && !FD_ISSET(fd, &write_sets))
						{
							QL_ADAPT_LOG("*** connect fail ***\r\n");
							res = STATE_PORT_NETWORK_CONNECT_FAILED;
						}
						else if(FD_ISSET(fd, &read_sets) && FD_ISSET(fd, &write_sets))
						{
							...
							{
								QL_ADAPT_LOG("*** connect fail, sock_err = %d, errno = %u ***\r\n", sock_error, lwip_get_error(fd));
								res = STATE_PORT_NETWORK_CONNECT_FAILED;
							}
						}
						else if(!FD_ISSET(fd, &read_sets) && FD_ISSET(fd, &write_sets))
            ...
            close(fd);
            QL_ADAPT_LOG("connect error, errno: %d\n", lwip_get_error(fd));
        }
    } 

    if (res < 0) {
        QL_ADAPT_LOG("fail to establish tcp , res: %d\r\n\r\n",res);
    } else {
        QL_ADAPT_LOG("success to establish tcp, fd=%d\n", *fd_out);
        res = STATE_SUCCESS;
    }
...
static int32_t _core_sysdep_network_tcp_establish(core_network_handle_t *network_handle)
{
    ...
    QL_ADAPT_LOG("establish tcp connection with server(host='%s', port=[%u])\n", network_handle->host, network_handle->port);
    ...
    if (res == STATE_PORT_NETWORK_DNS_FAILED && strlen(network_handle->backup_ip) > 0) {
        QL_ADAPT_LOG("using backup ip: %s\n", network_handle->backup_ip);
...
static int32_t _core_sysdep_network_udp_server_establish(core_network_handle_t *network_handle)
{
    ...
    if (0 != setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, &opt_val, sizeof(opt_val))) {
       QL_ADAPT_LOG("setsockopt(SO_REUSEADDR) falied, errno: %d\n", lwip_get_error(sockfd));
    ...
    if (-1 == bind(sockfd, (struct sockaddr *)&servaddr, sizeof(struct sockaddr_in))) {
        QL_ADAPT_LOG("bind(%d) falied, errno: %d\n", (int)sockfd, lwip_get_error(sockfd));
        //perror("bind(%d) error");
        close(sockfd);
    ...
    QL_ADAPT_LOG("success to establish udp, fd=%d\n", (int)sockfd);
                     ...
                     int depth, uint32_t *flags)
{
    ...
    QL_ADAPT_LOG("user ignore flags: 0x%08X, peer cert verify flags on curt way: 0x%08X", MBEDTLS_X509_BADCERT_NOT_TRUSTED, *flags);
...
//    mbedtls_platform_set_calloc_free(_core_mbedtls_calloc, _core_mbedtls_free);
    ...
    if (network_handle->cred->max_tls_fragment == 0) {
        QL_ADAPT_LOG("invalid max_tls_fragment parameter\n");
        return STATE_PORT_TLS_INVALID_MAX_FRAGMENT;
    }
    QL_ADAPT_LOG("establish mbedtls connection with server(host='%s', port=[%u], profile_idx=[%d])\n", network_handle->host, network_handle->port,network_handle->profile_idx);
    ...
    if (res < 0) {
        QL_ADAPT_LOG("mbedtls_ssl_conf_max_frag_len error, res: -0x%04X\n", -res);
        return res;
    }
#endif
    ...
    if (res < 0) {
        QL_ADAPT_LOG("mbedtls_ssl_config_defaults error, res: -0x%04X\n", -res);
        return res;
    }
    ...
    if (network_handle->cred->option == AIOT_SYSDEP_NETWORK_CRED_SVRCERT_CA) {
        if (network_handle->cred->x509_server_cert == NULL && network_handle->cred->x509_server_cert_len == 0) {
            QL_ADAPT_LOG("invalid x509 server cert\n");
        ...
        if (res < 0) {
            QL_ADAPT_LOG("mbedtls_x509_crt_parse server cert error, res: -0x%04X\n", -res);
            return STATE_PORT_TLS_INVALID_SERVER_CERT;
        }
        ...
        if (network_handle->cred->x509_client_cert != NULL && network_handle->cred->x509_client_cert_len > 0 &&
            ...
            if (res < 0) {
                QL_ADAPT_LOG("mbedtls_x509_crt_parse client cert error, res: -0x%04X\n", -res);
            ...
            if (res < 0) {
                QL_ADAPT_LOG("mbedtls_pk_parse_key client pk error, res: -0x%04X\n", -res);
            ...
            if (res < 0) {
                QL_ADAPT_LOG("mbedtls_ssl_conf_own_cert error, res: -0x%04X\n", -res);
                return STATE_PORT_TLS_INVALID_CLIENT_CERT;
            }
        }
    ...
    if (res < 0) {
        QL_ADAPT_LOG("mbedtls_ssl_setup error, res: -0x%04X\n", -res);
        return res;
    }
    ...
    if (_host_is_ip(network_handle->host) == 0) {
        ...
        if (res < 0) {
            QL_ADAPT_LOG("mbedtls_ssl_set_hostname error, res: -0x%04X\n", -res);
            return res;
        }
    }
    ...
    while ((res = mbedtls_ssl_handshake(&network_handle->mbedtls.ssl_ctx)) != 0) {
        if ((res != MBEDTLS_ERR_SSL_WANT_READ) && (res != MBEDTLS_ERR_SSL_WANT_WRITE)) {
            QL_ADAPT_LOG("mbedtls_ssl_handshake error, res: -0x%04X\n", -res);
    ...
    if (res < 0) {
        QL_ADAPT_LOG("mbedtls_ssl_get_verify_result error, res: -0x%04X\n", -res);
        return res;
    }

    QL_ADAPT_LOG("success to establish mbedtls connection, fd = %d(cost %d bytes in total, max used %d bytes)\n",
...
int32_t core_sysdep_network_establish(void *handle)
{
    ...
    QL_ADAPT_LOG("unknown nwk type or tcp host absent\n");
        ...
        uint32_t timeout_ms)
{
    ...
    fd_set recv_sets;
    ql_timeval_t timestart, timenow;
    ...
    /* Start Time */
    ql_gettimeofday(&timestart);
    timestart_ms = (uint64_t)timestart.sec * 1000 + (uint64_t)timestart.usec / 1000;
    timenow_ms = timestart_ms;

    do {
        ql_gettimeofday(&timenow);
        timenow_ms = (uint64_t)timenow.sec * 1000 + (uint64_t)timenow.usec / 1000;
        ...
        if (res == 0) {
            /* QL_ADAPT_LOG("_core_sysdep_network_tcp_recv, nwk select timeout\n"); */
            continue;
        } else if (res < 0) {
            QL_ADAPT_LOG("_core_sysdep_network_tcp_recv, errno: %d\n", lwip_get_error(network_handle->fd));
          //  perror("_core_sysdep_network_tcp_recv, nwk select failed: ");
            ...
            if (FD_ISSET(network_handle->fd, &recv_sets)) {
                ...
                if (recv_res == 0) {
                    QL_ADAPT_LOG("_core_sysdep_network_tcp_recv, nwk connection closed\n");
                    return STATE_PORT_NETWORK_RECV_CONNECTION_CLOSED;
                } else if (recv_res < 0) {
                    QL_ADAPT_LOG("_core_sysdep_network_tcp_recv, errno: %d\n", lwip_get_error(network_handle->fd));
                 //   perror("_core_sysdep_network_tcp_recv, nwk recv error: ");
                    if (lwip_get_error(network_handle->fd) == EINTR) {
                        continue;
                    }
                    return STATE_PORT_NETWORK_RECV_FAILED;
                } else {
                    recv_bytes += recv_res;
                    /* QL_ADAPT_LOG("recv_bytes: %d, len: %d\n",recv_bytes,len); */
                    if (recv_bytes == len) {
                        break;
                    }
                }
...
static int32_t _core_sysdep_network_udp_recv(core_network_handle_t *network_handle, uint8_t *buffer, uint32_t len, uint32_t timeout_ms, core_sysdep_addr_t *addr)
{
    ...
    if (res == 0) {
        QL_ADAPT_LOG("select timeout\n");
        return 0;
    }
    else if (res < 0) {
        QL_ADAPT_LOG("_linux_nwk_udp_read select errno: %d\n", lwip_get_error(network_handle->fd));
    ...
    if (res >= 0) {
        ...
        QL_ADAPT_LOG("_linux_nwk_udp_read errno: %d\n", lwip_get_error(network_handle->fd));
    ...
    do {
        ...
        if (res < 0) {
            if (res == MBEDTLS_ERR_SSL_TIMEOUT) {
                ...
                if (recv_bytes == 0) {
                    QL_ADAPT_LOG("mbedtls_ssl_recv error, res: -0x%04X\n", -res);
                                      ...
                                      uint32_t timeout_ms)
{
    ...
    /* Start Time */
    ql_gettimeofday(&timestart);
    timestart_ms = (uint64_t)timestart.sec * 1000 + timestart.usec / 1000;
    timenow_ms = timestart_ms;

    do {
        ql_gettimeofday(&timenow);
        timenow_ms = (uint64_t)timenow.sec * 1000 + timenow.usec / 1000;
        ...
        if (res == 0) {
            QL_ADAPT_LOG("_core_sysdep_network_tcp_send, nwk select timeout\n");
            continue;
        } else if (res < 0) {
            QL_ADAPT_LOG("_core_sysdep_network_tcp_send, errno: %d\n", lwip_get_error(network_handle->fd));
          //  perror("_core_sysdep_network_tcp_send, nwk select failed: ");
            ...
            if (FD_ISSET(network_handle->fd, &send_sets)) {
                ...
                if (send_res == 0) {
                    QL_ADAPT_LOG("_core_sysdep_network_tcp_send, nwk connection closed\n");
...
int32_t _core_sysdep_network_udp_send(core_network_handle_t *network_handle, uint8_t *buffer, uint32_t len, uint32_t timeout_ms, core_sysdep_addr_t *addr)
{
    ...
    if (addr == NULL) {
        QL_ADAPT_LOG("invalid parameter addr\n");
        return STATE_PORT_NETWORK_SEND_FAILED;
    }
    ...
    if (res == 0) {
        ...
        QL_ADAPT_LOG("_linux_nwk_udp_write select errno: %d\n", lwip_get_error(network_handle->fd));
    ...
    if (res < 0) {
        QL_ADAPT_LOG("sys_nwk_write, addr error\r\n");
        return STATE_PORT_NETWORK_SEND_FAILED;
    }
    ...
    if (res < 0) {
        QL_ADAPT_LOG("_linux_nwk_udp_write errno: %d\n", lwip_get_error(network_handle->fd));
        ...
        uint32_t timeout_ms)
{
    ...
    ql_timeval_t timestart, timenow;// timeout;
    ...
    do {
        ...
        if (res < 0) {
            if (res != MBEDTLS_ERR_SSL_WANT_READ &&
                res != MBEDTLS_ERR_SSL_WANT_WRITE) {
                if (send_bytes == 0) {
                    QL_ADAPT_LOG("mbedtls_ssl_send error, res: -0x%04X\n", -res);
                    if (res == MBEDTLS_ERR_SSL_PEER_CLOSE_NOTIFY) {
                                 ...
                                 core_sysdep_addr_t *addr)
{
    ...
    if (handle == NULL || buffer == NULL) {
        QL_ADAPT_LOG("invalid parameter\n");
        return STATE_PORT_INPUT_NULL_POINTER;
    }
    if (len == 0 || timeout_ms == 0) {
        return STATE_PORT_INPUT_OUT_RANGE;
...
void core_sysdep_rand(uint8_t *output, uint32_t output_len)
{
    ...
    srand((unsigned int)(time.sec * 1000 + time.usec / 1000) + rand());
