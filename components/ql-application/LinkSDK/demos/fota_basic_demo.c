...
int		fotafile_handle = 0;
static 	char *cur_version = "1.0.0";
char	fota_file_name[100] = "UFS:/fota.pack";
...
int fota_basic_image_verify( char *fotaFilePath);
...
static void user_download_recv_handler(void *handle, const aiot_download_recv_t *packet, void *userdata)
{
     ...
     */
	file_free_size = ql_fs_free_size(fota_file_name);
	if(file_free_size > packet->data.len)
	{
		ql_rtos_task_sleep_ms(1);
    	int ret = ql_fwrite(packet->data.buffer, packet->data.len,1,fotafile_handle);
		if(ret < 0)//-2: 表示下载失败
		{
    ...
    if (percent == 100) 
	{
		int ret = fota_basic_image_verify(fota_file_name);
		if(QL_FOTA_SUCCESS != ret)//-3: 表示校验失败
		{
			aiot_download_report_progress(handle, AIOT_OTAERR_CHECKSUM_MISMATCH);	
		}
        /* 上报版本号 */
        /*
...
static void user_ota_recv_handler(void *ota_handle, aiot_ota_recv_t *ota_msg, void *userdata)
{
    switch (ota_msg->type) {
        case AIOT_OTARECV_FOTA: {
			...
			cur_version = ota_msg->task_desc->version;
...
static void ql_alisdk_fota_task(void *arg)
{
	...
	if (fotafile_handle <= 0)
	{
		fotafile_handle = ql_fopen(fota_file_name, "w+");
		if(fotafile_handle < 0)
		{
			uart_printf("open file:[%s] fail fd:[%d]\r\n",fota_file_name,fotafile_handle);
			return ;
		}
	}
	/*执行拨号操作，设备联网*/
	...
	*/
	...
	if( QL_FOTA_SUCCESS == ql_fota_get_result(&p_fota_result) && QL_FOTA_READY == p_fota_result )//-1: 表示升级失败
	{
		uart_printf("Upgrade failed");
		aiot_download_report_progress(ota_handle, AIOT_OTAERR_UPGRADE_FAILED);
	}
    /*   TODO: 非常重要!!!
     *
    ...
    while (1) {
		...
		{
		    uart_printf("download failed, error code is %d, exit!\r\n", res);
			break;
		}
...
int fota_basic_image_verify(char *fotaFilePath)
{
	ql_errcode_fota_e ret = ql_fota_image_verify(fotaFilePath);
	if ( ret != QL_FOTA_SUCCESS )
	{
		ql_remove(fotaFilePath);
		uart_printf("[%s]package is invalid",fotaFilePath);	
	}
	else//校验成功
	{
		uart_printf("download is sucess ,system will reset power!");
