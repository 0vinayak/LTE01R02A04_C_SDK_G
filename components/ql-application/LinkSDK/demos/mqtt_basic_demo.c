...
static void demo_mqtt_default_recv_handler(void *handle, const aiot_mqtt_recv_t *packet, void *userdata)
{
    switch (packet->type) {
        ...
        case AIOT_MQTTRECV_PUB: {
            uart_printf("pub, qos: %d, topic_len %d: topic %s\n", packet->data.pub.qos, packet->data.pub.topic_len, packet->data.pub.topic);
...
//     
    ...
    res = ql_rtos_task_create(&ql_alisdk_mqtt_process_task_ref, 16 * 1024, 24, "client_process", demo_mqtt_process_thread, mqtt_handle,5);
    if (res < 0) {
    ...
    res = ql_rtos_task_create(&ql_alisdk_mqtt_recv_task_ref, 16 * 1024, 25, "client_recv", demo_mqtt_recv_thread, mqtt_handle,5);
    if (res < 0) {
...
exit1:
    ...
    if (ql_alisdk_mqtt_process_task_ref != NULL)
        ql_rtos_task_delete(ql_alisdk_mqtt_process_task_ref);
    if (ql_alisdk_mqtt_recv_task_ref != NULL)
