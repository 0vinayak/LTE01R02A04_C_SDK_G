...
ql_task_t ql_alisdk_rrpc_task_ref;
ql_task_t ql_alisdk_rrpc_mqtt_process_task_ref;
ql_task_t ql_alisdk_rrpc_mqtt_recv_task_ref;
static uint8_t g_rrpc_mqtt_process_task_running = 0;
static uint8_t g_rrpc_mqtt_recv_task_running = 0;
...
static int datacall_satrt(void)
{
    ...
    ql_set_data_call_asyn_mode(nSim, rrpc_profile_idx, 0);

    uart_printf("===start data call====");
    ret = ql_start_data_call(nSim, rrpc_profile_idx, QL_PDP_TYPE_IP, "uninet", NULL, NULL, 0);
    ...
    ret = ql_get_data_call_info(nSim, rrpc_profile_idx, &info);
    if (ret != 0)
    {
        uart_printf("ql_get_data_call_info ret: %d", ret);
        ql_stop_data_call(nSim, rrpc_profile_idx);
        goto exit;
    }
...
static void demo_mqtt_process_thread(void *args)
{
    ...
    while (g_rrpc_mqtt_process_task_running) {
	...
	ql_alisdk_rrpc_mqtt_process_task_ref = NULL;
...
static void demo_mqtt_recv_thread(void *args)
{
    ...
    while (g_rrpc_mqtt_recv_task_running) {
	...
	ql_alisdk_rrpc_mqtt_recv_task_ref = NULL;
	ql_rtos_task_delete(NULL);
    return;
}

static void ql_alisdk_rrpc_task(void *arg)
{
    int32_t     res = STATE_SUCCESS;
    ...
    */
    ...
    {
        char *sub_topic = "/a13FN5TplKq/mqtt_rrpc_demo/user/get";

        res = aiot_mqtt_sub(mqtt_handle, sub_topic, NULL, 1, NULL);
        if (res < 0) {
            uart_printf("aiot_mqtt_sub failed, res: -0x%04X\n", -res);
            goto exit;
        }
    }
    ...
    /* {
        ...
        if (res < 0) {
            uart_printf("aiot_mqtt_pub failed, res: -0x%04X\n", -res);
            goto exit;
        }
    } */

    /* 创建一个单独的线程, 专用于执行aiot_mqtt_process, 它会自动发送心跳保活, 以及重发QoS1的未应答报文 */
    g_rrpc_mqtt_process_task_running = 1;
    res = ql_rtos_task_create(&ql_alisdk_rrpc_mqtt_process_task_ref, 24576, 24, "client_test", demo_mqtt_process_thread, (void *)mqtt_handle, 5);
    if (res < 0) {
    ...
    g_rrpc_mqtt_recv_task_running = 1;
    res = ql_rtos_task_create(&ql_alisdk_rrpc_mqtt_recv_task_ref, 24576, 25, "recv_test", demo_mqtt_recv_thread, (void *)mqtt_handle, 5);
    if (res < 0) {
...
exit:
    g_rrpc_mqtt_process_task_running = 0;
    g_rrpc_mqtt_recv_task_running = 0;
	while(NULL != ql_alisdk_rrpc_mqtt_process_task_ref || NULL != ql_alisdk_rrpc_mqtt_recv_task_ref)
	{
		ql_rtos_task_sleep_s(1);
	}
...
int ql_ali_sdk_rrpc_test_init(void)
{
	QlOSStatus   status;
    int count = 0;
    /* 主循环进入休眠 */
	...
	status =ql_rtos_task_create(&ql_alisdk_rrpc_task_ref, QL_ALI_TASK_STACK_SIZE, APP_PRIORITY_NORMAL, "rrpc_task", ql_alisdk_rrpc_task, NULL, QL_ALI_TASK_EVENT_CNT);
