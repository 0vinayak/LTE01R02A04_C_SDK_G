...
//BLE SMP 配对
ql_timer_t ql_ble_smp_start_timer = NULL;                   //用来开启BLE SMP配对流程
int ql_ble_smp_dev_paired = 0;                              //0-表示建立连接的对端设备没有绑定过。1-表示对端设备已绑定
/*
...
{
    .io_cap = QL_SMP_DISPLAY_ONLY,                          //模块只有显示功能
...
void ql_ble_smp_start_pair_cb(void *ctx)
{
    if (ql_ble_smp_dev_paired == 1)
    {
        //已经配对过了，不需要处理
    }
    else
    {
        //设置SMP配置
        if (QL_BT_SUCCESS == ql_ble_smp_set_cfg(&ql_ble_smp_cfg))
        {
            //发起BLE SMP配对流程
            if (QL_BT_SUCCESS != ql_ble_smp_start_paring(0))
            {
                QL_BLE_GATT_LOG("pair start err");
            }
        }
    }
}
        ...
        switch (test_event.id)
        {
            ...
            case QUEC_BLE_CONNECT_IND:
            {
                if (QL_BT_STATUS_SUCCESS == status)
                {
                    ...
                    if (ret == QL_BT_SUCCESS)
                    {  
                        //客户应用确定是否需要发起BLE SMP配对，需要先检测下QUEC_BLE_GATT_SMP_COMPLETE_IND事件，判断是否已配对
                        //SMP配对由client发起，server端可以通知client发起SMP配对请求
                        if (ql_ble_smp_start_timer == NULL)
                        {
                            if (QL_OSI_SUCCESS == ql_rtos_timer_create(&ql_ble_smp_start_timer, ble_demo_task, ql_ble_smp_start_pair_cb, NULL))
                            {
                                ql_rtos_timer_start(ql_ble_smp_start_timer, 1000, 0);
                            }
                        }
                        else 
                        {
            ...
            {
                //BLE SMP配对流程已跑完
                QL_BLE_GATT_LOG("pair finish");
                if (test_event.param1 == 0)
                {
                    ql_ble_smp_dev_paired = 1;
                }
                else 
                {
                    QL_BLE_GATT_LOG("pair failed");
                }
            }
            break;
            case QUEC_BLE_GATT_SMP_USER_CONFIRM_IND:
            {
                ...
                if (user_info)
                {
                    QL_BLE_GATT_LOG("cid =%d,pair_mode=%d,PIN code=%d", user_info->cid, user_info->pair_mode, user_info->pin);
                    //pair_mode的取值进行相应的操作
            ...
            case QUEC_BLE_GATT_READ_RSSI_IND:
            {
                if (QL_BT_STATUS_SUCCESS == status)
                {
                    QL_BLE_GATT_LOG("read rssi sucess");
                    ql_bt_addr_s *addr = (ql_bt_addr_s *)test_event.param2;
                    int rssi = test_event.param3;
                    if (addr)
                    {
                        QL_BLE_GATT_LOG("rssi:%d", rssi);
                        free(addr);
                    }
                }
            }
