...
enum smd_ch_id
{
#ifdef CONFIG_IPC_PLAT_8850CM_V2
    SMD_CH_EPDC = 0, ///< EPDC, stream type
#endif
...
typedef struct
{
    uint32_t sysmail4_env;
    uint32_t sysmail5_env;
    uint32_t sysmail6_env;
    uint32_t sysmail7_env;
    uint32_t sysmail8_env;
    uint32_t sysmail9_env;
    uint32_t sysmail30_env;
} sysmail_env_t;

sysmail_env_t gSysmailEnvInfo;

#ifdef CONFIG_IPC_PLAT_8850CM_V2
/**
 * IPC MSG QUEUE for 8850CMv1.1, multiplexing AT1 channel
 */
#define SMD_CH_IPC_QUEUE SMD_CH_AT1
/**
 * PS Node Block Pool for 8850CMv1.1, multiplexing PS channel
 */
#define SMD_CH_NODE_POOL SMD_CH_PS

/* PS NODE BLOCK POOL define */
#define PS_NODE_UL_CNT 680 /* 60B*680 */
#define PS_NODE_UL_SIZE 64 //60
#define PS_NODE_UL_MASK (PS_NODE_UL_CNT - 1)
#define PS_NODE_UL_BUF_SIZE (680 * PS_NODE_UL_SIZE)

#define PS_NODE_DL_BCNT 1056
#define PS_NODE_DL_BSIZE 32 //16
#define PS_NODE_DL_BMASK (PS_NODE_DL_BCNT - 1)
#define PS_NODE_DL_BBUF_SIZE (PS_NODE_DL_BCNT * PS_NODE_DL_BSIZE)

#define PS_NODE_DL_LCNT 768
#define PS_NODE_DL_LSIZE 32 //8
#define PS_NODE_DL_LMASK (PS_NODE_DL_LCNT - 1)
#define PS_NODE_DL_LBUF_SIZE (PS_NODE_DL_LCNT * PS_NODE_DL_LSIZE)
#endif
...
int32_t ipc_ch_close(struct smd_ch *ch);

/**
 * @brief reset nv ipc buf
 *
 * @param ch    nv ipc channle
 */
void ipc_reset_nv_buf(struct smd_ch *ch);

#ifdef CONFIG_GSM_LTE_SWITCH_SUPPORT
/**
 * @brief notify cp enter GSM/LTE switch process
 * @param rat      GSM or LTE id
 */
void ipc_notify_rat_switch(uint8_t rat);

/**
 * @brief check rat switch prepare ready
 * @return ready result
 */
bool ipc_check_ratswitch_prepare(void);

/**
 * @brief set rat swtich as default
 */
void ipc_set_ratswitch_flag(void);

/**
 * @brief get idle_res5 value, for gsm/lte switch
 * @return hwp_idle->idle_res5 value
 */
uint32_t ipc_get_idleres5_value(void);
#endif

#if defined(CONFIG_SOC_8850)
void hal_SimIrqHandlerAp(uint32_t interruptId);
void hal_lteF8f9Lisr(int x);
#endif
