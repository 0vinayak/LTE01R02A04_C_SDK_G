...
typedef struct
{
    ...
    /** refer to \p drvDebugPortWorkEnable */
    void (*work_enable)(struct drvDebugPort *d, bool enable);
} drvDebugPortOps_t;
 ...
 * \brief work enable
 *
 * It may be called for the registered debugHost port at \p osiTraceSetPushEnable.
 *
 * \param d debug port instance, must be valid
 * \param enable debughost to tx log from ifc to fifo output
 */
void drvDebugPortWorkEnable(drvDebugPort_t *d, bool enable);
 ...
 *      - debug uart instance
 */
drvDebugPort_t *drvDebugUartPortCreate(unsigned name, drvDebugPortMode_t mode, unsigned baud);
...
/**
 * \brief detection 8910 debughost interrupt
 */
void drvDhostDetection(void);
