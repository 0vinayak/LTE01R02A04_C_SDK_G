...
// Determine whether the target_freq for rc26m calib needs to be updated
bool targetFreqNeedUpdate;
...
typedef enum
{
    DRV_UART_DATA_BITS_5 = 5,
    DRV_UART_DATA_BITS_6 = 6,
...
typedef void (*drvUartHandshakeCB_t)(void *param, bool ret);
...
{
    uint32_t baud_max;
    bool data_bit5_support : 1;
    bool data_bit6_support : 1;
    bool data_bit7_support : 1;
    bool data_bit8_support : 1;
    bool stop_bit1_support : 1;
    bool stop_bit2_support : 1;
    bool parity_odd_support : 1;
    bool parity_even_support : 1;
    bool parity_mark_support : 1;
    bool parity_space_support : 1;
} drvUartAttr_t;
 ...
 *
 * When the UART is already opened, it will return true directly.
 *
...
/**
 * @brief create an UART driver for already opened device
 *
 * This is similar to \p drvUartCreate and \p drvUartOpen. And it will only
 * create and open already enabled device. Inside, UART configuration won't
 * be changed. So, UART configuration related fields will be ignored, and
 * will be filled with the actual value.
 *
 * This will be useful when the data already in hardware rx fifo shall be
 * kept.
 *
 * @param name  FOURCC UART name (DRV_NAME_UART{1, 2, 3})
 * @param cfg   UART config
 * @return
 *      - created UART driver
 *      - NULL on fail, invalid parameter or UART isn't enabled.
 */
drvUart_t *drvUartOpenKeepConfig(uint32_t name, drvUartCfg_t *cfg);
 ...
 *
 * @param uart  the UART driver
 * @return uart device name
 */
uint32_t drvUartGetName(drvUart_t *uart);
...
/**
 * @brief get the UART attribute
 *
 * @param uart  the UART driver
 * @return
 *      - NULL  fail, only when \p uart is NULL
 *      - other the UART attribute struct address
 */
const drvUartAttr_t *drvUartAttribute(drvUart_t *uart);

/**
 * @brief get the UART attribute from the device name
 *
 * @param name  the UART device name
 * @return
 *      - NULL  invalid device name
 *      - other the UART attribute struct address
 */
const drvUartAttr_t *drvUartAttributeFromName(uint32_t name);
...
/**
 * @brief set event mask and callback
 *
 * @param uart  the UART driver
 * @param event_mask event mask, identify which event will be notified
 * @param event_cb event notify callback function
 * @param event_cb_ctx event callback context
 * @return
 *      - true on sucess
 *      - false on fail, invalid parameter
 */
bool drvUartSetCallback(drvUart_t *uart, uint32_t event_mask, drvUartEventCB_t event_cb, void *event_cb_ctx);
