...
typedef struct
{
    ...
    void (*resume)(struct composite_func_s *f);
    void (*suspend)(struct composite_func_s *f);
} copsFuncOpt_t;
...
/**
 * @breif composite function suspend
 *
 * @param f     the function
 */
static inline void cfSuspend(copsFunc_t *f)
{
    if (CHECK_CF_OPS(f, suspend))
        f->ops.suspend(f);
}

/**
 * @breif composite function resume
 *
 * @param f     the function
 */
static inline void cfResume(copsFunc_t *f)
{
    if (CHECK_CF_OPS(f, resume))
        f->ops.resume(f);
}
...
/**
 * @brief cops api for mtp notify vfs add file
 *
 * @param c the composite driver
 */
void copsMtpNotify(cops_t *c);
...
#ifdef CONFIG_QUEC_PROJECT_FEATURE
int copsAssignInterface_ex(cops_t *c, copsFunc_t *f);
int quec_usb_assign_interface(uint8 intf, cops_t *c, copsFunc_t *f);
#endif
...
/**
 * @brief get rndis netcard if transfer success
 *
 * @param c     the composite device
 * @return
 *      - NonNull if transfer success
 */
bool copsRndisTimeoutNotify(cops_t *c);

/**
 * @brief get ecm netcard if transfer success
 *
 * @param c     the composite device
 * @return
 *      - NonNull if transfer success
 */
bool copsEcmTimeoutNotify(cops_t *c);

/**
 * @brief get ecm if active success
 *
 * @param c     the composite device
 * @return
 *      - NonNull if active success
 */
bool copsEcmNotifyState(cops_t *c);
 ...
 *      - NonNull if active success
 */
bool prvEcmNotifyState(copsFunc_t *f);

/**
 * @brief get rndis if active success
 *
 * @param c     the composite device
 * @return
 *      - NonNull if active success
 */
bool copsRndisNotifyState(cops_t *c);
 ...
 *      - NonNull if active success
 */
bool prvRndisNotifyState(copsFunc_t *f);
