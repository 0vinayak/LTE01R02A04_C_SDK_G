...
void drvHostRmcHandler(drvHostCmdEngine_t *cmd, uint8_t *packet, unsigned packet_len)
{
    uint8_t id = osiBytesGet8(packet + 4);
    uint8_t fctrl = osiBytesGet8(packet + 5);
    uint16_t bitmap = osiBytesGetLe16(packet + 6);

    (void)id;

    // ignore command to set bitmap for id
    if (fctrl == 0)
        return;
    ...
    // [3]: RELEASE_TX
    if ((bitmap & 0xc) == 0x4)
        osiTraceSetOutputEnable(false);
    else if ((bitmap & 0xc) == 0x8)
