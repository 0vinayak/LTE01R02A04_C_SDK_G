...
static const cameraReg_t RG_InitQvgaMIPI[] =
{
    ...
    {0x41, 0x21},
    {0x42, 0xcf},
    {0x44, 0x00}, //The register sets the output format of yuv. 0x00 indicates CbYCrY,0x02 indicates YCbYCr.
    {0x45, 0xa8},
    {0x46, 0x02}, //sync
    {0x4a, 0x11},
    {0x4b, 0x01},
...
static const cameraReg_t RG_InitQvgaSPI[] =
{
    {0xfe,0xf0},
    {0xfe,0xf0},
    {0xfe,0x00},
...
static const cameraReg_t RG_InitVgaMIPI[] =
    {
        ...
        {0x44, 0x02}, //The register sets the output format of yuv. 0x00 indicates CbYCrY,0x02 indicates YCbYCr.
#else 
        {0x44, 0x00},
#endif           
        {0x45, 0xa8},
...
#ifdef CONFIG_MIPI_SUPPORT
        camCsi_In, //     camCsi_In if mipi
#else
        camSpi_In,
#endif
...
static bool prvCamGc0310Rginit(sensorInfo_t *info)
{
    ...
    case camSpi_In: 
		...
		{
            if (!prvCamWriteRegList(RG_InitQvgaSPI, sizeof(RG_InitQvgaSPI) / sizeof(cameraReg_t)))
            {
                return false;
            }
		}
...
void camGc0310TestMode(bool on)
{
    sensorInfo_t *p = &gc0310Info;
    if (p->isCamOpen)
    {
        if (on)
        {
            ...
            //prvCamWriteOneReg(0xfa, 0x32);
            prvCamWriteOneReg(0x40, 0x08);
            prvCamWriteOneReg(0x41, 0x00);
            prvCamWriteOneReg(0x42, 0x00);
            prvCamWriteOneReg(0x4f, 0x00);
            prvCamWriteOneReg(0x18, 0x06);
            prvCamWriteOneReg(0x77, 0x40);
            prvCamWriteOneReg(0x78, 0x40);
            prvCamWriteOneReg(0x79, 0x40);
            prvCamWriteOneReg(0xd0, 0x40);
            prvCamWriteOneReg(0xdd, 0x00);
            prvCamWriteOneReg(0x70, 0x40);
            prvCamWriteOneReg(0x71, 0x20);
            prvCamWriteOneReg(0x72, 0x40);
            prvCamWriteOneReg(0x03, 0x00);
            prvCamWriteOneReg(0x04, 0x00);
            prvCamWriteOneReg(0x4c, 0x01);
        }
        else
        {
            prvCamWriteOneReg(0xfe, 0x00);
            prvCamWriteOneReg(0x4c, 0x20);
        }
    }
}
...
bool quec_camGc0310CheckId(void)
{
    uint32_t i2c_name[2] = {DRV_NAME_I2C1, DRV_NAME_I2C2};
    uint8_t i;
    uint8_t sensorID[2] = {0, 0};
    sensorInfo_t *gc0310Info;
    gc0310Info = quec_getGc0310Info();
    if (!gc0310Info->isCamOpen)
    {
        ...
        for(i=0;i<2;i++)
        {
            sensorID[0] = 0;
            sensorID[1] = 0;
            if (!prvCamGc0310I2cOpen(i2c_name[i], gc0310Info->baud))
                continue;
            ...
            camGc0310GetId(sensorID, 2);
            OSI_LOGI(0,"gc0310 get id 0x%x,0x%x", sensorID[0], sensorID[1]);
            if ((gc0310Info->sensorid[0] == sensorID[0]) && (gc0310Info->sensorid[1] == sensorID[1]))
            {
                gc0310Info->i2c_name = i2c_name[i];
                camGc0310Close();
                return true;
            }
        }
...
bool camGc0310Reg(SensorOps_t *pSensorOpsCB)
{
    ...
    if (ret)
    {
        ...
        pSensorOpsCB->cameraTestMode = camGc0310TestMode;
        return true;
    }
    return false;
}
