...
#endif
#ifdef CONFIG_QUEC_PROJECT_FEATURE_CAMERA_BF3095
extern bool CamBf3095Reg(SensorOps_t *pSensorOpsCB);
#endif
...
#else
    //关闭camera使能与释放camera分成两部分，避免ImageSensor.poweron_flag为false时，无法释放内存
    drvCamPowerOff();
    if (ImageSensor.poweron_flag == false)
    {
        for (uint8_t i = 0; i < ImageSensor.framebuffer_count; i++)
