...
sensorInfo_t gc032aInfo =
    {
        ...
        320,              //        uint16_t spi_pixels_per_line;   // max width of source image
        240,              //        uint16_t spi_pixels_per_column; // max height of source image
#else
...
#ifdef __QUEC_OEM_VER_AC__
        320,              //		uint16_t dstWinColEnd;
#else
        640,              //		uint16_t dstWinColEnd;
#endif
        0,                //		uint16_t dstWinRowStart;
#ifdef __QUEC_OEM_VER_AC__
        240,              //		uint16_t dstWinRowEnd;
#else
...
void camGc032aTestMode(bool on)
{
    if (on == true)
    {
        prvCamWriteOneReg(0x4c, 0x8);
    }
}
...
bool quec_camGc032aCheckId(void)
{
    uint32_t i2c_name[2] = {DRV_NAME_I2C1, DRV_NAME_I2C2};
    uint8_t i;
    uint8_t sensorID[2] = {0, 0};
    sensorInfo_t *gc032aInfo;
    gc032aInfo = quec_getGc032aInfo();
    if (!gc032aInfo->isCamOpen)
    {
        drvCamSetPdn(false);
        osiDelayUS(5);
        prvCamGc032aPowerOn();
        drvCamSetMCLK(gc032aInfo->sensorClk);
        drvCamSetPdn(true);
        osiDelayUS(5);
        drvCamSetPdn(false);
        osiDelayUS(5);
        ...
        for(i=0;i<2;i++)
        {
            sensorID[0] = 0;
            sensorID[1] = 0;
            if (!prvCamGc032aI2cOpen(i2c_name[i], gc032aInfo->baud))
                continue;
            ...
            camGc032aGetId(sensorID, 2);
            OSI_LOGI(0,"gc032a get id 0x%x,0x%x", sensorID[0], sensorID[1]);
            if ((gc032aInfo->sensorid[0] == sensorID[0]) && (gc032aInfo->sensorid[1] == sensorID[1]))
            {
                gc032aInfo->i2c_name = i2c_name[i];
                camGc032aClose();
                g_quec_camera_type = QL_CAM_TYPE_GC032A;
                return true;
            }
        }
        camGc032aClose();
        OSI_LOGI(0,"check id error");
        return false;
    }
    else
    {
        OSI_LOGI(0,"camera is opened");
        return false;
    }
}
...
bool camGc032aReg(SensorOps_t *pSensorOpsCB)
{
    ...
    if (ret)
    {
        ...
        pSensorOpsCB->cameraTestMode = camGc032aTestMode;
        return true;
    }
    return false;
}
