...
#include "ql_usb.h"
extern ql_usb_setting_s quec_usb_current_setting;
#endif
...
#ifdef CONFIG_QUEC_PROJECT_FEATURE_USB_CHARGE      
        CHGMNG_ADP_STANDARD, //adp_type  插入USB开机不能识别adp_type，导致使用CHGMNG_ADP_UNKNOW，插拔USB后，
        					 //又会变成CHGMNG_ADP_STANDARD充电，所以此处统一修改为标准充电
#else
...
{
    // return drvUsbIsConnected();
    return false;
}

int32_t drvAdcGetChannelVoltStub(uint32_t channel, int32_t scale)
{
    int32_t vol;
    vol = (int32_t)drvAdcGetChannelVolt(channel, scale);
    return vol;
}
...
static void _updateChargerStatus(drvChargerContext_t *p)
{
    ...
    REG_PMIC_ANA_CHGR_STATUS_T status = {halAdiBusRead(&hwp_pmicAna->chgr_status)};
    p->plugged = level;
    if (!level)
    {
    ...
    OSI_LOGD(0x10007d95, "chg: level/%d status/%x type/%d", level, status.v, p->type);
...
{
    REG_PMIC_ANA_CHGR_CTRL1_T chargCtrol_1;
    halAdiBusBatchChange(
        &hwp_pmicAna->chgr_ctrl1, REG_FIELD_MASKVAL1(chargCtrol_1, chgr_cc_en, 1),
...
{
    REG_PMIC_RTC_ANA_CHGR_CTRL0_T chargCtrol0;
    if (charge_stop == true)
        return;
    halAdiBusBatchChange(
        &hwp_pmicRtcAna->chgr_ctrl0, REG_FIELD_MASKVAL1(chargCtrol0, chgr_pd, 0),
        HAL_ADI_BUS_CHANGE_END);

    OSI_LOGI(0x10007d19, "chg: Turn On charger");
}
...
static void _drvChargerTurnOff(void)
{
    REG_PMIC_RTC_ANA_CHGR_CTRL0_T chargCtrol0;
    halAdiBusBatchChange(
        &hwp_pmicRtcAna->chgr_ctrl0, REG_FIELD_MASKVAL1(chargCtrol0, chgr_pd, 1),
        HAL_ADI_BUS_CHANGE_END);
    OSI_LOGI(0x10007d1a, "chg: Turn Off charger");
}
...
#define CHGR_ITERM_0P1 1
    ...
    REG_PMIC_ANA_CHGR_CTRL1_T chargCtrol_1;
    OSI_LOGI(0x10007d96, "chg: SetChargeEndCurrent %d", ma);
    chargCtrol_1.v = halAdiBusRead(&hwp_pmicAna->chgr_ctrl1);
    ...
    halAdiBusBatchChange(
        &hwp_pmicAna->chgr_ctrl1, REG_FIELD_MASKVAL1(chargCtrol_1, chgr_iterm, b),
        HAL_ADI_BUS_CHANGE_END);
}
...
static
#endif
void _drvChargerSetEndVolt(uint32_t mv)
{
...
#define CHGR_END_V_4500 4500
    ...
    uint32_t b;
    OSI_LOGI(0x10007d97, "chg: SetChargeEndVolt %d", mv);
    ...
    b &= 3;
    halAdiBusBatchChange(
        &hwp_pmicAna->chgr_ctrl1, REG_FIELD_MASKVAL1(chargCtrol_1, chgr_end_v, b),
        HAL_ADI_BUS_CHANGE_END);
}
...
static
#endif
 void _drvChargerSetChangerCurrent(uint16_t current)
{
    uint32_t temp = 0;
    REG_PMIC_ANA_CHGR_CTRL1_T cc_reg;

    OSI_LOGI(0x10007d98, "chg: SetChgCurrent=%d", current);
    ...
    halAdiBusBatchChange(
        &hwp_pmicAna->chgr_ctrl1, REG_FIELD_MASKVAL1(cc_reg, chgr_cc_i, temp),
        HAL_ADI_BUS_CHANGE_END);
}
...
static void _drvChargerSetSwitchOverPoint(uint16_t eswitchpoint)
{
    ...
    OSI_LOGI(0x10007d1d, "chg: SetSwitchoverPoint", eswitchpoint);

    REG_PMIC_RTC_ANA_CHGR_CTRL0_T chargCtrol0;
    halAdiBusBatchChange(
        &hwp_pmicRtcAna->chgr_ctrl0, REG_FIELD_MASKVAL1(chargCtrol0, chgr_cv_v, eswitchpoint),
        HAL_ADI_BUS_CHANGE_END);
}
/*****************************************************************************/
...
static void _drvAdaptorSendMsgToChgTask(chgMsg_t chgMsg, uint32_t event_param)
{
    ...
    if (chgMsg == CHG_CHARGER_PLUG_IN_MSG)
        OSI_LOGV(0x10007d20, "chg: Send Msg CHG_CHARGER_PLUG_IN_MSG");
    if (chgMsg == CHG_CHARGER_PLUG_OUT_MSG)
        OSI_LOGV(0x10007d21, "chg: Send Msg CHG_CHARGER_PLUG_OUT_MSG");

    OSI_LOGD(0x10007d99, "chg: Send adaptor Msg To chg thread %d", chgMsg);
...
static void _drvMonitorSendMsgToChgTask(chgMsg_t chgMsg, uint32_t event_param)
{
    ...
    if (chgMsg == CHG_VBAT_MONITOR_MSG)
        OSI_LOGV(0x10007d1e, "chg: Send Msg CHG_VBAT_MONITOR_MSG");

    OSI_LOGD(0x10007d9a, "chg: Send bat monitor Msg To chg thread %d", chgMsg);
    while (false == osiThreadCallback(gDrvChargeCtx.chgTaskId, _chargerServerThread, d))
        ;
}
#endif
...
static void _drvChargerSendMsgToChgTask(chgMsg_t chgMsg, uint32_t event_param)
{
    ...
    if (chgMsg == CHG_CHARGER_MONITOR_MSG)
        OSI_LOGV(0x10007d1f, "chg: Send Msg CHG_CHARGER_MONITOR_MSG");
    if (chgMsg == CHG_MODULE_RESET_MSG)
        OSI_LOGV(0x10007d22, "chg: Send Msg CHG_MODULE_RESET_MSG");

#ifdef BATTERY_DETECT_SUPPORT
    if (chgMsg == CHG_BATTERY_OFF_MSG)
        OSI_LOGV(0x10007d23, "chg: Send Msg CHG_BATTERY_OFF_MSG");
#endif
    if (chgMsg == CHG_MAX_MSG)
        OSI_LOGV(0x10007d24, "chg: Send Msg CHG_MAX_MSG");

    OSI_LOGD(0x10007d9b, "chg: Send Charging Msg To chg thread %d", chgMsg);
    while (false == osiThreadCallback(gDrvChargeCtx.chgTaskId, _chargerServerThread, d))
        ;
}
...
{
    return halAdiBusRead(&hwp_pmicAna->chgr_status);
}
...
static bool _drvchargerGetCVStatus(void)
{
    ...
    REG_PMIC_ANA_CHGR_STATUS_T chg_status;
    chg_status = (REG_PMIC_ANA_CHGR_STATUS_T)_drvChgGetStatus();
...
static uint16_t _drvChargerAdcValueToCurrent(uint32_t voltage)
{
    ...
    OSI_LOGD(0x10007d26, "chg:  ADCValueToCurrent current_type= %d, current:%d\n", current_type, temp);
...
static uint32_t _drvChargerGetVprogAdcResult(void)
{
    ...
    {
        vprog_result[i] =
#ifndef  CONFIG_QUEC_PROJECT_FEATURE_USB_CHARGE //修改参考电压量程，使电流AD值和实际值相符合
            drvAdcGetChannelVoltStub(ADC_CHANNEL_PROG2ADC, ADC_SCALE_0);
#else
            drvAdcGetChannelVoltStub(ADC_CHANNEL_PROG2ADC, ADC_SCALE_2);
#endif
...
static uint32_t _drvChargerGetChgCurrent(void)
{
    ...
    OSI_LOGD(0x10007d9c, "chg: _drvChargerGetChgCurrent:progAdcResult:%d,chgCurrent:%d\n", progAdcResult, chgCurrent);
    return chgCurrent;
}
...
static uint32_t _drvChargerGetADCResultByChannel(uint32_t adc_channel, int32_t scale)
{
    ...
    {
        vbat_result[i] = drvAdcGetChannelVoltStub(adc_channel, scale);
        OSI_LOGD(0x10007d9d, "chg: _GetADCResult i:%d adc:%d", i, vbat_result[i]);
    }
...
static uint32_t _drvChargerGetChannelVol(uint32_t adc_channel, int32_t scale)
{
    ...
    OSI_LOGD(0x10007d9e, "chg: etChannelVol adc_channel:%d vol=%d", adc_channel, vol);
...
static int32_t _drvChargerGetVbatTempVolValue(void)
{
    ...
    vol = _drvChargerGetChannelVol(ADC_CHANNEL_BAT_DET, ADC_SCALE_0);
    ...
    OSI_LOGD(0x10007d9f, "chg: VbatTempVolValue temp=%d, vol=%d", temp, vol);
...
OSI_UNUSED static uint32_t _drvChargerGetCurrentTemp(void)
{
    ...
    OSI_LOGD(0x10007da0, "chg: GetCurrentTemp = %d", result);
...
static void _drvChargerStartCharge()
{
    ...
    OSI_LOGI(0x10007d28, "chg: ChargeStartHandler!");

    if (_drvChargerCheckVbatPresent() == 0)
    {
        _drvChargerTurnOff();

        OSI_LOGI(0x10007d29, "chg: StartCharge NO BAT");
        return;
    }
...
#endif
    {
        OSI_LOGE(0x10007d2a, "chg: Invalid application timer pointer!");
        g_charge_plus = 0;
    }
    g_charge_plus = 1;
...
{
    OSI_LOGI(0x10007d2b, "chg: StopCharge!");
    ...
    case CHG_PESUDO_CHARGERUNPLUG:
        ...
        {
            OSI_LOGI(0x10007d2a, "chg: Invalid application timer pointer!");
        }

        OSI_LOGI(0x10007d2c, "chg: CHR_CHARGE_DISCONNECT1!!!!!!!!!!!!!!!!!!!!!!!!!!!");
    ...
    case CHG_CHARGEDONE:
        OSI_LOGI(0x10007d2d, "chg: CHR_CHARGE_FINISH!!!!!!!!!!!!!!!!!!!!!!!!!!!");
        _drvChargerSendMsgToClient(CHR_CHARGE_FINISH, 0);
        break;
...
{
    OSI_LOGI(0x10007d2e, "chg: FSMProcess fsm_event:%d", fsm_event);
    ...
    default:
        OSI_LOGE(0x10007d2f, "chg: Error chgFsmEvent!!!");
        osiPanic();
        break;
    }
}
...
{
    OSI_LOGD(0x10007d30, "chg: module_state.chgmng_state:%d", module_state.chgmng_state);
    OSI_LOGD(0x10007d31, "chg: .bat_statistic_vol:%d", module_state.bat_statistic_vol);
    OSI_LOGD(0x10007d32, "chg: .bat_cur_vol:%d", module_state.bat_cur_vol);
    OSI_LOGD(0x10007d33, "chg: .bat_remain_cap:%d", module_state.bat_remain_cap);

    if (_drvChargerIsPresent())
    {
        OSI_LOGD(0x10007d34, "chg: .charging_current:%d", module_state.charging_current);
        OSI_LOGD(0x10007d35, "chg: .adp_type:%d", module_state.adp_type);
        OSI_LOGD(0x10007d36, "chg: .charging_stop_reason:%d", module_state.charging_stop_reason);
        OSI_LOGD(0x10007d37, "chg: hw_switch_point:%d", hw_switch_point);
        OSI_LOGD(0x10007d38, "chg: chg_end_vol:%d", chg_param.chg_end_vol);
    }
    else
    {
        OSI_LOGD(0x10007d33, "chg: .bat_remain_cap:%d", module_state.bat_remain_cap);
        OSI_LOGD(0x10007d39, "chg: dischg_param.shutdown_vol:%d", dischg_param.shutdown_vol);
        OSI_LOGD(0x10007d3a, "chg: .deadline_vol:%d", dischg_param.deadline_vol);
        OSI_LOGD(0x10007d3b, "chg: .warning_vol:%d", dischg_param.warning_vol);
    }
    OSI_LOGD(0x10007d3c, "chg: info vbat_queue.pointer:%d", vbat_queue.pointer);
}
...
static uint32_t _drvChgGetVol(void)
{
    ...
    vol = drvAdcGetChannelVoltStub(ADC_CHANNEL_VCHGSEN, ADC_SCALE_0);
    if (vol == -1)
    {
        osiThreadSleep(2);
    ...
    OSI_LOGI(0x10009f5f, "chg: get charger vol:%d ", vol);
    return (uint32_t)vol;
}
...
static uint32_t _drvChargerGetVbatVol(void)
{
    ...
    vol = (uint32_t)drvAdcGetChannelVolt(ADC_CHANNEL_11, ADC_SCALE_3);
    ...
    OSI_LOGI(0x10009f60, "chg: get battery vol:%d ", vol);
    return vol;
}
...
{
    REG_PMIC_ANA_CHGR_STATUS_T reg_sta;

    reg_sta = (REG_PMIC_ANA_CHGR_STATUS_T)_drvChgGetStatus();
    return false;
...
static uint32_t _drvChargerVoltagePercentum(uint32_t voltage, chgState_e is_charging, bool update)
{
...
#ifdef CONFIG_QUEC_PROJECT_FEATURE_USB_CHARGE
    if ((pre_percentum == 0xffff) || (pre_percentum - percentum) > 5)
#else
    ...
    OSI_LOGD(0x10007d3e, "chg: bat percent : %d,dischg_bat_capacity_table[2][0]:%d,dischg_bat_capacity_table[11][0]:%d",
...
static void _drvChargerVbatMonitorRoutine()
{
    ...
    if (_drvChargerCheckVbatPresent() == 0)
    {
        OSI_LOGI(0x10007d3f, "chg: _drvChargerVbatMonitorRoutine NO BAT");
        return;
    }

//#ifdef _DETEDCT_BATTERY_TEMP_  //quectel update
#ifndef CONFIG_QUEC_PROJECT_FEATURE_USB_CHARGE
    //drvChargerGetCurrentTemp();
    //OSI_LOGI(0x10007da3, "chg: charging_temperature = %d", module_state.charging_temperature);
#endif
    ...
    if (module_state.charging_temperature >= HOT_TEMP) //
    {
        ...
        if (module_state.chgmng_state == CHG_CHARGING)
        {
            ...
            OSI_LOGI(0x10007da4, "chg: stop charging for over temperaure");
        }
    }
    else
    {
#ifndef CONFIG_QUEC_PROJECT_FEATURE_USB_CHARGE
        OSI_LOGI(0x10007da5, "chg: low temperaure %d", module_state.chgmng_state);
        if (module_state.chgmng_state == CHG_IDLE)
#else
		if ((module_state.chgmng_state == CHG_IDLE) && (module_state.charging_stop_reason == CHG_OVERTEMP))
#endif
        {
            OSI_LOGI(0x10007da6, "chg: start charging for over temperaure");
...
//#endif  //quectel update
    ...
    if (module_state.chgmng_state == CHG_IDLE)
    {
        ...
        {
            OSI_LOGI(0x10007da7, "chg: adc error -1");
        }
        ...
        if (module_state.bat_statistic_vol < dischg_param.warning_vol)
        {
            ...
            {
                OSI_LOGI(0x10007d40, "chg: CHR_WARNING_IND!!!");
                _drvChargerSendMsgToClient(CHR_WARNING_IND, 0);
            }
                ...
                (module_state.bat_cur_vol < dischg_param.deadline_vol))
            {
                if (0 == warning_counter)
                {
                    OSI_LOGI(0x10007d41, "chg: WARNING_IND Before shutdown!!!");
                    _drvChargerSendMsgToClient(CHR_WARNING_IND, 0);
                    _CHG_COUNTER_ADD(warning_counter);
                }
                else
                {
                    OSI_LOGI(0x10007d42, "chg:  SHUTDOWN_IND!!!");
    ...
    osiTimerStartRelaxed(gDrvChargeCtx.batMonTimer, CHG_BAT_MONITOR_PERIOD_, OSI_WAIT_FOREVER);
    OSI_LOGI(0x10007d43, "chg: Vbat Monitor next ! bat capicity = %d", module_state.bat_remain_cap);
}
...
static void _drvChargerMonitorChargerRoutine(void)
{
...
#ifdef CONFIG_QUEC_PROJECT_FEATURE_USB_CHARGE    
    if(0 == quec_usb_current_setting.charging_off_time)
    {
    ...
    if (_drvChargerCheckVbatPresent() == 0)
    {
        OSI_LOGI(0x10007d44, "chg: _drvChargerStartCharge NO BAT");
#ifndef CONFIG_QUEC_PROJECT_FEATURE_USB_CHARGE            
    ...
    OSI_LOGI(0x10007d45, "chg:  Monitor  bat_statistic_vol =%d,  batVol =%d ,chgVol=%d, rechg_vol=%d, ovp_resume_vol=%d, isChgOVP=%d, isBatOVP=%d",
    ...
    if (_drvChargerVBatIsOV() || _drvChargerVChgIsOV() || (isChgOVP == true) || (isBatOVP == true))
    {
        ...
        OSI_LOGI(0x10007d46, "chg: ChargeMonitorRoutine over voltage");
    ...
    else
    {
        if (module_state.charging_stop_reason == CHG_OVERVOLTAGE)
        {
            if (chgVol < ovp_param.ovp_resume_vol)
            {
                ...
                OSI_LOGI(0x10007d47, "chg: ChargeMonitorRoutine resume chgVol=%d  ", chgVol);
                _drvChargerFSMProcess(CHG_FSM_EVENT_START_CHG, 0);
            }
    ...
    OSI_LOGI(0x10007d48, "chg: ChargeMonitorRoutine chgmng_state=%d, g_charge_plus =%d", module_state.chgmng_state, g_charge_plus);
    ...
    if (module_state.chgmng_state == CHG_IDLE)
    {
        ...
        if (module_state.bat_statistic_vol <= chg_param.rechg_vol)
        {
            ...
            OSI_LOGI(0x10007d49, "chg: ChargeMonitorRoutine recharge vot=%d  ", module_state.bat_statistic_vol);
            _drvChargerFSMProcess(CHG_FSM_EVENT_START_CHG, 0);
        }

        return;
    }
    OSI_LOGI(0x10007d4a, "chg: ChargeMonitorRoutine plus= %d", g_charge_plus);
    ...
    else
    {
        ...
        OSI_LOGI(0x10007d4b, "chg: update batVol = %d", batVol);
        _drvChargerVbatQueueUpdate(module_state.bat_cur_vol);
    ...
    if (_drvchargerGetCVStatus())
    {
        ...
        OSI_LOGI(0x10007d4c, "chg: ChargeMonitorRoutine charger full, and wating  cv_status_counter =%d ", cv_status_counter);
...
static void _chargerServerThreadPro(chgMsg_t *chg_msg)
{
    ...
    case CHG_IDLE_MSG:
        break;
    default:
        OSI_LOGI(0x10007d4e, "chg: Error msg!!!");
        break;
    }
...
uint32_t _drvChargerAdcValutToChgVol(uint32_t adc)
{
    ...
    result = drvAdcGetChannelVoltStub(ADC_CHANNEL_VCHGSEN, ADC_SCALE_0);

    OSI_LOGI(0x10009f61, "chg: _drvChargerAdcValutToChgVol result=%d", result);
...
{
    OSI_LOGE(0x10007da9, "chg: WarningVoltage Setting is too low!");
...
static void _drvChargerSetVBatCapTable(uint16_t *dischg_ptable, uint16_t *chg_ptable)
{
    ...
    OSI_LOGD(0x10007daa, "chg: dischg_bat_capacity_table[5][0]:%d\n", dischg_bat_capacity_table[5][0]);
    OSI_LOGD(0x10007dab, "chg: dischg_bat_capacity_table[6][0]:%d\n", dischg_bat_capacity_table[6][0]);
    OSI_LOGD(0x10007dac, "chg: chg_bat_capacity_table[5][0]:%d\n", chg_bat_capacity_table[5][0]);
    OSI_LOGD(0x10007dad, "chg: chg_bat_capacity_table[6][0]:%d\n", chg_bat_capacity_table[6][0]);
}
...
static void _drvChargePluginHandler(void)
{
    OSI_LOGI(0x10007d50, "chg: _drvChargePluginHandler ischgmng_start=%d", ischgmng_start);
...
void drvChargerInit(void)
{
    ...
    REG_PMIC_RTC_ANA_CHGR_STATUS_T chgr_status;
    OSI_LOGI(0x10007dae, "chg: drvChargerInit!");

    _drvChargetPhyInit(); //Charger control initial setup
    cv_v = _drvChgGetEffuseBits(14);
    OSI_LOGI(0x10007daf, "chg: get cv_v is 0x%x", cv_v);
    ...
    OSI_LOGI(0x10007d52, "chg: drvChargerInit bat_cur_vol =%d", module_state.bat_cur_vol);
    ...
    if (module_state.bat_cur_vol < dischg_param.shutdown_vol)
    {
        ...
        {
            //PowerOff();
            OSI_LOGI(0x10007d53, "chg: power down because low battery volt");
        }
    }

    ischgmng_start = true; //Charge management has started.
    halAdiBusBatchChange(
        &hwp_pmicRtcAna->chgr_status,
        REG_FIELD_MASKVAL2(chgr_status, chgr_int_en, 0, dcp_switch_en, 1),
        HAL_ADI_BUS_CHANGE_END);
    drvPmicEicSetCB(DRV_PMIC_EIC_CHGR_INT, _chargeIsrCB, NULL);
    drvPmicEicTrigger(DRV_PMIC_EIC_CHGR_INT, CHARGER_DEBOUNCE, true);
    _updateChargerStatus(p);

    OSI_LOGI(0x10007db0, "chg: drvChargerInit end");
}
...
{
	chg_param.rechg_vol               = charge_config->rechg_vol;
	chg_param.chg_end_vol             = charge_config->chg_end_vol;
	chg_param.nonstandard_chg_current = charge_config->standard_chg_current;
	chg_param.chg_timeout             = charge_config->chg_timeout;
    _drvChargerSetEndVolt(chg_param.chg_end_vol);
    _drvChargerSetChangerCurrent(chg_param.nonstandard_chg_current); // Set default current    
}

void quec_charge_get_config(ql_charge_config_t *charge_config)
{
	 charge_config->rechg_vol            = chg_param.rechg_vol;
	 charge_config->chg_end_vol          = chg_param.chg_end_vol;
	 charge_config->standard_chg_current = chg_param.nonstandard_chg_current;
	 charge_config->chg_timeout          = chg_param.chg_timeout;
}
...
void quec_charge_set_over_temp(bool over_temp)
{
	if(true == over_temp)
	{
		module_state.charging_temperature = HOT_TEMP + 1;
	}
	else
	{
		module_state.charging_temperature = 0;
	}
}
