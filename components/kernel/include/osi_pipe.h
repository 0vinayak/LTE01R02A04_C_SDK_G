...
#if ((defined CONFIG_QUEC_PROJECT_FEATURE_AUDIO) || (defined CONFIG_QUEC_PROJECT_FEATURE_PWM_AUDIO_DEC))
void osiPipeReload(osiPipe_t *pipe);
void osiPipeDataEnd(osiPipe_t *pipe);
int osiPipeDataIsEnd(osiPipe_t *pipe);
#if (defined CONFIG_QUEC_PROJECT_FEATURE_PWM_AUDIO_DEC)
//与osiPipeRead/osiPipeReadAll类似,只是直接把数据跳过
int osiPipeJump(osiPipe_t *pipe,unsigned size);
int osiPipeJumpAll(osiPipe_t *pipe,unsigned size, unsigned timeout);
#endif
#endif
