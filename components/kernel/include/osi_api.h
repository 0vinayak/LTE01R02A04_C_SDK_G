...
typedef struct
{
    uint8_t csclk_mode;
    uint8_t sh_psmode;
    uint8_t lps_psmthd_min;
    uint8_t cfun_flag; //bit0:sim0 cfun flag,bit1:sim1 cfun flag
    uint32_t lps_sleephold_pm1;
    uint32_t lps_sleephold_pm1plus;
    uint32_t lps_sleephold_pm2;
    uint32_t lps_resume_time_pm1;
    uint32_t lps_resume_time_pm1plus;
    uint32_t lps_resume_time_pm2;
    uint32_t lps_sleephold_pm3_min;
    uint32_t pmsource; //bit[0]:F8,bit[1]:SIM
} CP_SH_LPS_INFO;
...
/**
 * Support insert direction to message queue
 */
#define OSI_MSG_QUEUE_SEND_TO_BACK (0)
#define OSI_MSG_QUEUE_SEND_TO_FRONT (1)
...
#ifdef CONFIG_8850CM_TARGET
    OSI_PRIORITY_NORMAL_PLUS = 30,
#endif
    OSI_PRIORITY_ABOVE_NORMAL = 32,
#ifdef CONFIG_8850CM_TARGET
    OSI_PRIORITY_ABOVE_NORMAL_PLUS = 36,
#endif
 ...
 *
 * The typical method to support this feature in underlay RTOS is to get
 * stack base
 *
...
osiThread_t *osiWorkQueueGetThread(osiWorkQueue_t *wq);
...
/**
 * put a message to message queue, support insert direction(front or back).
 *
 * \p msg should hold content size the same as \p msg_size specified at
 * \p osiMessageQueueCreate.
 *
 ...
 * \param mq        message queue pointer
 * \param msg       mesage pointer
 * \param dirction  insert direction
 * \return
 *      - true on success
 *      - false on invalid parameter
 */
...
/**
 * put a message to message queue with timeout, support insert direction(front or back).
 *
 * This can be called in ISR. And in ISR, \p timeout will be ignored.
 *
 * \param mq        message queue pointer
...
/**
 * \brief get psm wake time  in 10 milliseconds
 *
 * \return      ap deep sleep time
 */
uint32_t osiGetPsmWakeTimeReg(void);
...
/**
 * start AP bus monitor
 *
 * Count the read and write command and data access of the 5 masters
 * (A5, CE, EMMC, LZMA, USB) on AP, support only on SOC8850
 */
void osiBusmonStart(void);

/**
 * stop AP bus monitor
 */
void osiBusmonStop(void);

/**
 * start memory monitor
 *
 * Panic when write to the address in range from memStart to memEnd
 * It is recommended to use it only for debug.
 *
 * \param memStart memory start address
 * \param memEnd   memory end address
 */
void osiMemMonitorStart(uintptr_t memStart, uintptr_t memEnd);
