...
/**
 * \brief monoclinic system hardware tick,exclude gOsiUpTimeOffset.
 *
 * **Don't** call this in application code. It is only for 8850CMV1.1 cp codes.
 *
 * \return      monoclinic system hardware tick,exclude gOsiUpTimeOffset.
 */
uint32_t osiHWTick32(void);

/**
 * \brief monoclinic system hardware tick(to ms),exclude gOsiUpTimeOffset.
 *
 * **Don't** call this in application code. It is only for 8850CMV1.1 cp codes.
 *
 * \return      monoclinic system hardware tick,exclude gOsiUpTimeOffset.
 */
uint32_t osiHWTick32MS(void);
 ...
 *
 * It is for debug only. The data format of pm source information dump is
 * not stable, and may change. When \p mem is NULL, it will return the
 * estimated dump size.
 *
...
/**
 * get active pm sources
 *
 * It should be only used for DEBUG.
 *
 * \param tags      output active pm source tags
 * \param count     maximum output count
 * \return
 *      - real written active pm source count
 */
int osiGetActivePmSources(uint32_t *tags, unsigned count);

/**
 * get clk_sys request sources
 *
 * It should be only used for DEBUG.
 *
 * \param tags      output clk_sys request tags
 * \param count     maximum output count
 * \return
 *      - real written clk_sys request count
 */
int osiGetSysClkRequestSources(uint32_t *tags, unsigned count);
