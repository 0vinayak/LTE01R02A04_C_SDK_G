...
typedef struct 
{
    ...
    uint32              eventPendingCnt;   /*Thread event suspend count*/
} ql_task_status_t;
...
*/
#define QL_FREE_PTR(p)    {if(p != NULL) {free(p);p = NULL;}}
#define ql_free(p)    	  QL_FREE_PTR(p)
...
//The minimum event id value is 10 
 ...
 *
 */
extern void ql_rtos_scheduler_suspend(void);

/**
 * resume thread scheduler
 *
 * Scheduler suspend and resume is not *recursive*. That is, after
 * \p ql_rtos_scheduler_resume is called, scheduler is resumed no matter
 * how many times \p ql_rtos_scheduler_suspend are called.
 */
extern void ql_rtos_scheduler_resume(void);

/**
 * create a thread with specified stack
 *
 * It is similar to \p ql_rtos_task_create, and the stack won't be dynamic created.
 * Rather, the specified buffer \p stack will be used at the stack of the
 * thread. 
 *
 */
extern QlOSStatus ql_rtos_task_create_with_stack
(
	ql_task_t	 * taskRef, 			 /* OS task reference										   				  */
...
/*****************************************************************
* Function: ql_app_image_from_flash
*
* Description:
*   APPIMG jump function loaded by flash, used to jump between multiple independent apps
*   通过flash加载方式的APPIMG跳转函数,用于多个独立APP之间跳转
* Parameters:
*   flash_address   [in]   APPIMG flash partition starting address,4KB aligned. APPIMG flash分区的起始地址,4KB对齐
*   flash_size      [in]   APPIMG flash partition size,4KB aligned. APPIMG flash分区的大小,4KB对齐
*   ram_address     [in]   APPIMG ram starting address. APPIMG ram空间的起始地址
*   ram_size        [in]   APPIMG ram size. APPIMG ram空间的大小
*   handler         [in]   appImageHandler_t APPIMG handle returned after parsing. APPIMG 解析后返回的句柄
* Return:
*   FASLE, APPIMG 信息检查或则解析出错 
*   TRUE , APPIMG 信息校验和解析成功
*
*****************************************************************/
bool ql_app_image_from_flash(const unsigned flash_address,const unsigned flash_size,const unsigned ram_address,const unsigned ram_size, void *handler);

/*****************************************************************
* Function: ql_app_image_from_file
*
* Description:
*   APPIMG jump function loaded by file, used to jump between multiple independent apps
*   通过file加载方式的APPIMG跳转函数,用于多个独立APP之间跳转
* Parameters:
*   fname           [in]   The pathname of the appimg file.APPIMG 文件所在的路径名.
*                          ex:CONFIG_APPIMG_LOAD_FILE_NAME \ CONFIG_APPIMG2_LOAD_FILE_NAME
*   ram_address     [in]   APPIMG ram starting address. APPIMG ram空间的起始地址
*   ram_size        [in]   APPIMG ram size. APPIMG ram空间的大小
*   handler         [in]   appImageHandler_t APPIMG handle returned after parsing. APPIMG 解析后返回的句柄
* Return:
...
*
*****************************************************************/
bool ql_app_image_from_file(const char *fname,const unsigned ram_address,const unsigned ram_size, void *handler);
