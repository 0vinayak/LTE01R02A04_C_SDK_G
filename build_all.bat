...
if /i "%ql_buildproj%" == "EC800GCN_GA" (
...
) else if /i "%ql_buildproj%" == "EG800GCN_GD" (
	set hardware_type=UIS8850BM
) else if /i "%ql_buildproj%" == "EC800GCN_MD" (
	set hardware_type=UIS8850BM
) else if /i "%ql_buildproj%" == "EC800GCN_ND" (
	set hardware_type=UIS8850BM
) else if /i "%ql_buildproj%" == "EG800GEU_LD" (
	set hardware_type=UIS8850BM
) else if /i "%ql_buildproj%" == "EG800GLA_LD" (
	set hardware_type=UIS8850BM
) else if /i "%ql_buildproj%" == "EC800GCN_LB" (
	set hardware_type=UIS8850CM
) else if /i "%ql_buildproj%" == "EG800GEU_LB" (
	set hardware_type=UIS8850CM
	...
	set hardware_type=UIS8850BM
) else if /i "%ql_buildproj%" == "EC600GEU_LD" (
	set hardware_type=UIS8850BM
	...
	set hardware_type=UIS8850CM
) else if /i "%ql_buildproj%" == "EC200GCN_LM" (
	set hardware_type=UIS8850CM
) else if /i "%ql_buildproj%" == "EC200GCN_LA" (
	set hardware_type=UIS8850CM
) else if /i "%ql_buildproj%" == "EC200GCN_GK" (
	set hardware_type=UIS8850CM
) else if /i "%ql_buildproj%" == "EG915GEU_GD" (
	set hardware_type=UIS8850BM
) else (
...
if /i "%hardware_type%"=="UIS8850BM" (
	...
	set BUILD_TARGET=8850CM_cat1_open
)
...
if /i "%hardware_type%"=="UIS8850DG" (
	...
	set partitionfile=components/hal/config/8850/partinfo_8850_4m_opencpu_novolte_v2.json
	set modemdir=8850BM_cat1bis_ds_plus
) else if /i "%hardware_type%"=="UIS8850CM" (
	if /i "%volte_enable%"=="VOLTE" (
        set partitionfile=components/hal/config/8850/partinfo_8850_8m_opencpu_v2.json
        set modemdir=8850CM_cat1bis_volte_ds_plus
        set ims_delta_nv=y
        set quec_ims_feature=y
    ) else (
        set partitionfile=components/hal/config/8850/partinfo_8850_8m_opencpu_novolte_v2.json
        set modemdir=8850CM_cat1bis_ds_plus
    )
)
...
copy /y "components\ql-config\build\public\delta_static_common.nv" "components\nvitem\8850\default\deltanv\"
copy /y "components\ql-config\build\public\delta_ims_common.nv" "components\nvitem\8850\default\deltanv\"
copy /y "components\ql-config\build\public\delta_simcard2_common.nv" "components\nvitem\8850\default\deltanv\"
...
::如果没生成merge.pac,并且报failed to copy APPIMG before PREPACK as APPIMG,或者预置文件失败等,要检查下ql_prepack.opt和prepack_json_path是否正确
    ..\..\tools\win32\dtools pacmerge --id APPIMG,CP --id PREPACK,FLASH %version_path%\%BUILD_TARGET%_%buildver%.pac %version_path%\app\%ql_app_ver%.pac %version_path%\%BUILD_TARGET%_%buildver%_merge.pac
) else (
...
if exist %version_path%\app\%ql_app2_ver%.pac (
