...
* descriptions (array)
  * flash block device
    ...
    * bootloader_format_on_fail: (optional) flag for format on fail in bootloader
    * application_format_on_fail: (optional) flag for format on fail in application
  * SFFS file system
    * type: SFFS
...
BOOTLOADER_FORMAT_ON_FAIL = 0x10
APPLICATION_FORMAT_ON_FAIL = 0x20
    ...
    def storebin(self, fname):
        ...
        for p in self._des['descriptions']:
            ...
            if dtype == 'FBD1' or dtype == 'FBD2':
                flags = 0
                if p.get('bootloader_ignore', False):
                    flags |= BOOTLOADER_IGNORE
                if p.get('application_ignore', False):
                    flags |= APPLICATION_IGNORE
                if p.get('bootloader_format_on_fail', False):
                    flags |= BOOTLOADER_FORMAT_ON_FAIL
                if p.get('application_format_on_fail', False):
                    flags |= APPLICATION_FORMAT_ON_FAIL
                ...
                flags = 0
                if p.get('bootloader_ro', False):
                    flags |= BOOTLOADER_RO
                if p.get('application_ro', False):
