...
def nvdata_modify(nvdata, nvid, offset, dlen, ddata):
    ...
    while pos < size:
        ...
        nvsize_aligned = (nvsize + 3) & ~3
        pos += 4 + nvsize_aligned
        # print("nvm nvid {} size {} align {}".format(bin_nvid, nvsize, nvsize_aligned))
...
# Add lz4_file to partition
def pgen_add_lz4(pgen, local_name, fs_name):
    if 'lz4_file' not in pgen:
        pgen['lz4_file'] = []
    pgen['lz4_file'].append({'file': fs_name, 'local_file': local_name})
...
def create_prepack_cpio(pc, outfname):
    with open(outfname, 'wb') as fh:
        for fm in pc:
            name = fm['file'].replace('\\', '/').replace('//', '/').lstrip('/').encode('utf-8')
...
def mimggen8910_args(sub_parser):
    ...
    parser.add_argument('--block-size', dest='block_size', default=None,
                        help='lzmar block size')
    parser.add_argument('--dict-size', dest='dict_size', default=None,
                        help='lzmar dictionary size')
    ...
    parser.add_argument('--cpsign', dest='cpsign', action='store_true',
                        help='sign cp images')
    parser.add_argument('--indeltanv', dest='indeltanv', default=None,
                        help='output indeltanv cpio file')
    parser.add_argument('--pn', dest='pn', help='product for vlrsign')
    parser.add_argument('--pw', dest='pw', help='password for vlrsign')
...
def mimggen8910(args):
    ...
    cpindeltanvdir = os.path.join(cpdir, 'indeltanv')
    ...
    indeltanvs = glob.glob(os.path.join(cpindeltanvdir, '*.nv'))
    ...
    for f in cpfiles:
        ...
        elif args.cpsign and cpfile in ['cp.bin', 'zsp.bin', 'bcpu_gsm.bin', 'riscV.bin']:
            subprocess.run(['vlrsign', '--pn', args.pn, '--pw', args.pw,
                            '--ha', 'Blake2', '--img', f, '--out', new_name])
        else:
    ...
    # Generate inner deltanv use all indelta*.nv under cpindeltanvdir
    if args.indeltanv:
        dinvdir = os.path.join(workdir, 'indeltanv')
        ensure_dir(dinvdir)
        invdnvgen = []

        for inv in indeltanvs:
            invname = os.path.basename(inv)
            invname = invname.split('.')[0]
            invdir = os.path.join(dinvdir, invname)
            ensure_dir(invdir)

            subprocess.run(['arm-none-eabi-gcc', '-E', '-P', '-x', 'c',
                    '-I', args.deltainc, inv, '-o', os.path.join(invdir, os.path.basename(inv))])
            subprocess.run(['NVGen', nvmprj, '-L', '-c', invdir])
                            #stdout=subprocess.DEVNULL)
            local = os.path.join('factory',invname+'.bin')
            rname = local.replace('\\', '/').replace('//', '/').lstrip('/')

            invdnvgen.append({'file': rname,
                        'local_file': os.path.join(invdir, 'delta_nv.bin')})
            #quectel updated
            #修复因为indeltanv中多出索引头导致合入失败，无法注网问题
            # Write empty Index.xml, to avoid annoying message from NVGen
            #with open(os.path.join(invdir, 'Index.xml'), 'w') as fh:
                #fh.write('<Operators version="1"/>\n')

        create_prepack_cpio(invdnvgen, args.indeltanv)
    ...
    for nv in nvbins:
        ...
        pgen_add_lz4(pgen, nv, os.path.join(cpcfg['modemnv_dir'], nvbase))
    ...
    if ims_nv_exists:
        ...
        pgen_add_lz4(pgen, os.path.join(imsgendir, 'delta_nv.bin'),
                      'nvm/ims_delta_nv.bin')

    # get modem_nvm.json default lzma_block_size and lzma_dict_size
    lzma_block_size = cpcfg['lzma_block_size']
    lzma_dict_size = cpcfg['lzma_dict_size']

    # judge lzma_block_size and lzma_dict_size is equal to the default value
    if args.block_size and args.dict_size:
        block_size = args.block_size
        dict_size = args.dict_size
        if (block_size != '0x10000'):
            lzma_block_size = block_size
        if (dict_size != '0x400'):
            lzma_dict_size = dict_size

    pgen['lzma_block_size'] = lzma_block_size
    pgen['dict-size'] = lzma_dict_size
...
def mvargen8850_args(sub_parser):
    parser = sub_parser.add_parser(
        'vargen8850', help='generate modem.cpio and nvitem.bin')
    parser.set_defaults(func=mvargen8850)
    parser.add_argument('--config', dest='config', required=True,
                        help='the json file for modem and nvm')
    parser.add_argument('--source-top', dest='srctop', required=True,
                        help='SOURCE_TOP_DIR, may be used')
    ...
    parser.add_argument('--aproot', dest='aproot', required=True,
                        help='root directory of AP nvm')
    # quectel modify
    parser.add_argument('--ims-region', dest='imsregion', required=True,
                        help='root directory of ims delta nvm')
    parser.add_argument('--prj', dest='prj', required=True,
                        help='QUEC PROJECT DEF')
    ...
    parser.add_argument('--prepackfile', dest='prepackfile', required=True,
                        help='the json file for prepack')
    parser.add_argument('--imsdeltanv', dest='imsdeltanv', default=None,
                        help='output imsdeltanv cpio file')
    parser.add_argument('--indeltanv', dest='indeltanv', default=None,
                        help='output indeltanv cpio file')
    parser.add_argument('--outHexDir', dest='outHexDir', default=None,
                        help='output out hex dir')
    parser.add_argument('--signcpbin', dest='signcpbin', default=None,
                        help='enable cp bin sign')
    parser.add_argument('--cpsimlockimg', dest='cpsimlockimg', default=None,
                        help='assign simlock key directory')
    parser.add_argument('--apbin', dest='apbin', default=None,
                        help='extract from elf for flash compressed')   
    parser.add_argument('--apcpio', dest='apcpio', default=None, 
                        help='output ap cpio file')                        
    parser.add_argument('variant', help='variant name')
    parser.add_argument('prepack', help='output prepack cpio file')
    parser.add_argument('nvbin', help='output nvitem bin')
    parser.add_argument('modem', help='output modem cpio file')


def mvargen8850(args):
    # Variant description json file name
    varjson = os.path.join(args.aproot, 'config', args.variant + '.json')
    ...
    workdir = args.workdir
    outHexDir = args.outHexDir
    signcpbin = args.signcpbin
    cpsimlockimg = args.cpsimlockimg
    cpdir = os.path.join(args.cproot, vari['cpdir'])
    apdir = os.path.join(args.aproot, vari['apdir'])
    cpdnvdir = os.path.join(cpdir, vari['cpdeltanv'])
    # Fixed file name by convention
    ...
    lzma_dict_size = cpcfg['lzma_dict_size']
    # quectel modify
    imsdir = os.path.join(args.imsregion, 'ims_delta_nv')
    cpindeltanvdir = os.path.join(cpdir, 'indeltanv')
    cpdnvin = os.path.join(cpdnvdir, 'modem_config.nv')

    # lzmar block size
    if args.block_size:
        block_size = str(int(args.block_size, 16))
        lzma_block_size = block_size

    # lzmar dict size
    if args.dict_size:
        dict_size = str(int(args.dict_size, 16))
        lzma_dict_size = dict_size
    ...
    imsnvms = glob.glob(os.path.join(imsdir, '*.nv'))
    imsxml = os.path.join(imsdir, 'Index.xml')
    # quectel modify 减少打包的indeltanv的数量，只针对对应X00G进行打包
    indeltanvs = glob.glob(os.path.join(cpindeltanvdir, '*_' + args.prj + '.nv'))
    ...
    for f in cpfiles:
        ...
        if cpfile == 'memory_index_list.bin':
            ...
            if signcpbin == 'true':
            #deal cp-sign file
                cpsignfile = os.path.join(cpbindir, 'cp-sign.img')
                print ("Do cpio file with: [cp-sign.img]")
                shutil.copy(cpsignfile, new_name)
            else:
                if cpsimlockimg != 'NULL':
                #deal cp-simlock file
                    cpsignfile = os.path.join(cpbindir, 'cp-simlock.bin')
                    print("Do cpio file:[cp-simlock.bin]")
                    shutil.copy(cpsignfile, new_name)
                else:
                    print("Do cpio file:[" + cpfile + "]")
                    shutil.copy(f, new_name)
        else:
            shutil.copy(f, new_name)

    # copy ap_compress.bin to workdir and make cpio file.
    if(args.apbin):
        print("args.apbin: " + args.apbin)
    if(args.apcpio):
        print("args.apcpio: " + args.apcpio)
    if(args.apbin):
        apbin = args.apbin
        apbindir = os.path.join(workdir, 'apbin')
        # delte all cache files in apbin
        if apbindir and os.path.exists(apbindir):
            shutil.rmtree(apbindir)
        ensure_dir(apbindir)
        new_name = os.path.join(apbindir, os.path.basename(args.apbin))
        shutil.copy(apbin, new_name)
        
        apbingen = []
        fname = os.path.join('modem', os.path.basename(new_name))
        print("fname: " + fname)
        print("new_name: " + new_name)
        for bfname in glob.glob(new_name + '.???'):
            os.unlink(bfname)
        subprocess.run(['dtools', 'lzmare3', '--block-size', lzma_block_size,
                        lzma_block_size, new_name, new_name, '--dict-size', lzma_dict_size])
        suffixes = [x.split('.')[-1]
                    for x in glob.glob(new_name + '.???')]
        for s in sorted(suffixes):
            apbingen.append({'file': '{}.{}'.format(fname, s),
                          'local_file': '{}.{}'.format(new_name, s)})
        if(args.apcpio):                  
            create_prepack_cpio(apbingen, args.apcpio) 
    ...
    # Generate delta_nv.bin use all *.nv under dnvdir, it will merged
    # according to the order of *.nv file name
    subprocess.run(['NVGen', nvmprj, '-L', '-c', dnvdir])
    ...
    # modem cpio configuration
    cpgen = []
    for fm in cpfilecfg:
        if fm.get('copy_only', False):
            continue

        fname = os.path.join('modem', fm['file'])
        local_fname = os.path.join(cpbindir, fm.get('local_file', fm['file']))
        if fm.get('lzma3', False):
            for bfname in glob.glob(local_fname + '.???'):
                os.unlink(bfname)
            subprocess.run(['dtools', 'lzmare3', '--block-size', lzma_block_size,
                            lzma_block_size, local_fname, local_fname, '--dict-size', lzma_dict_size])
            suffixes = [x.split('.')[-1]
                        for x in glob.glob(local_fname + '.???')]
            for s in sorted(suffixes):
                cpgen.append({'file': '{}.{}'.format(fname, s),
                              'local_file': '{}.{}'.format(local_fname, s)})
        else:
            cpgen.append({'file': fname, 'local_file': local_fname})

    # generate modem image json
    create_prepack_cpio(cpgen, args.modem)
    ...
    if args.imsdeltanv:
        ...
        imsdnvgen = []
        imsdnvgen.append({'file': '/factory/ims_delta_nv.bin',
                          'local_file': os.path.join(imsgendir, 'delta_nv.bin')})
        create_prepack_cpio(imsdnvgen, args.imsdeltanv)
